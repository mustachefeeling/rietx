...
# Raised again for the magnetic family's own routing row (small-fixes-20260917,
# item 3, issue #286): every magnetic and distortion-mode diagnostic moved out
# of §7/§7g/§9b into its own §7j file (references/magnetic.md; §7i when it
# landed, renumbered when WP-1415's reader file took §7i), which needed
# one new index-table row here to be reachable at all
# (test_every_reference_file_is_reachable_from_the_body). The body was at
# 32_993 B, 7 B under the old cap — nowhere near enough room for a row the
# size of its neighbours — and the row is not droppable without breaking that
# reachability test, so the cap moves instead. 33_600 keeps a similar margin
# above the new body to the one 33_000 kept above 32_993; the technique's
# generated index (references/api-magnetic.md) takes the second of the two
# rows, and the body landed at 33_348 B.
SKILL_MAX_BYTES = 33_600
...
#: `diagnostics.md` the room WP-1338 deliberately denied it.
...
def test_every_dotted_name_in_the_api_index_resolves():
    """An API index cannot name something the package does not have.

    The union over `api.md` and every `api-<technique>.md`: a technique index
    is generated by the same generator and is as much a door as the everyday
    one, so a name that no longer resolves has to fail here wherever it was
    routed."""
    names: set[str] = set()
    for index in API_INDEXES:
        text = index.read_text(encoding="utf-8")
        found = {m.rstrip(".") for m in DOTTED.findall(text)}
        for name in sorted(found):
            dotted = (name if name.startswith("rietx.")
                      else "rietx." + name[len("rx."):])
            resolve_dotted(dotted, index.name)
        names |= found
...
def test_the_api_index_is_what_the_generator_renders():
    ...
    # every index the generator renders, not only the everyday one: a
    # technique index is generated and committed on the same terms, and the
    # set is read off the generator so a new one cannot be left unchecked.
    assert sorted(mod.indexes()) == sorted(p.name for p in API_INDEXES), (
        "the generator's indexes and the committed api*.md files disagree — "
        "regenerate, then re-sync the copies")
    for name in mod.indexes():
        rendered = mod.render(name)
        committed = (REFERENCE_DIR / name).read_text(encoding="utf-8")
        if rendered != committed:
            diff = list(difflib.unified_diff(
                committed.splitlines(), rendered.splitlines(),
                "committed", "rendered", lineterm="", n=0))
            pytest.fail(
                f"references/{name} is stale — regenerate with\n"
