...
def _state_magnetic_width():
    ...
    the two new width terms are derivative paths no other row covers: they
    ...
    by name: WP-1327's traced twin refuses any model carrying a moment (the
...
CONFIGS = {"families": _state_families,
           ...
           "magnetic_width": _state_magnetic_width, **STATES}
...
def test_the_magnetic_config_declines_the_traced_backends_by_name():
    ...
    WP-1343's config is the only magnetic model in this file, and its autodiff
    ...
    """
    model, table = _state("magnetic_width")
    assert any(cp.magnetic is not None for cp in model.phases)
    assert model.mag_split(0), "the second frozen family was not built"
    asked = 0
