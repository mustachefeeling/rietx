...
def _distortion() -> tuple[object, object]:
    """A Pnma parent restated with the displacive modes of k = (½,0,0).

    Same reason as :func:`_magnetic` one function up: a distortion-mode
    amplitude contributes no dot-path at all unless a phase declares a mode, so
    a manual naming ``phases.*.distortion_modes.*.amplitude`` would fail the
    check below against every other model — for the right reason, on a real
    path.  Built through the builder rather than hand-written, so the path
    shape is the one a real statement produces.
    """
    ...
    statement = displacive_statement(parent, ("1/2", "0", "0"), irrep="S2",
                                     direction="(rank 1)#2")
    return (Structure(phases=[statement.phase]),
            Instrument.constant_wavelength_neutron(2.4))


def _parameter_paths() -> set[str]:
    """Every dot-path five representative models put on the table.
    ...
    fully-declared LaB6 above for the optional correction blocks, a
    magnetic rutile for the moment DOFs, and a Pnma child carrying displacive
    distortion modes for their amplitudes.
    """
    plain = Instrument.debye_scherrer(wavelength=0.4139)
    models = [(make_lab6(), plain), (make_rutile(vary_coords=True), plain),
              _fully_declared(), _magnetic(), _distortion()]
    paths: set[str] = set()
...
def test_every_figure_reference_exists():
    """A Part 1 figure is a committed file, not a build product, so a renamed
    or unregenerated one is a broken image and nothing else notices: sphinx
    warns on a missing image, but the figures are generated by
    `docs/manual/make_figures.py` and a stale reference to a file that still
    exists builds clean and shows the wrong picture.  This catches the half a
    build can catch, by name."""
    for page, text in _pages():
        targets = [
            *re.findall(r"^```\{(?:image|figure)\}\s+(\S+)\s*$", text, re.M),
            *re.findall(r"!\[[^\]]*\]\(([^)\s]+)\)", text),
        ]
