...
| `11BM_LaB6_cBN_mg2044.xye` | NIST **SRM 660b LaB₆** + cubic BN two-phase mixture, APS beamline 11-BM, λ = 0.413680 Å (from the accompanying `.prm` ICONS record), 49 496 points, 0.5–49.99° 2θ at 0.001°, three-column `xye` with **propagated esds** (twelve analyser crystals summed, so column 3 is not √I) — the two-phase QPA and identifiability acceptance (`test_acceptance_lab6_cbn.py`). **No weighed composition is recorded anywhere in the source folder**, so every claim referenced to this file is cross-code against the two TOPAS `.inp` refinements that ship beside it (LaB₆ 17.907 and 17.950 wt %, cBN a = 3.616463 and 3.616466 Å) | Owner's own measurement, APS 11-BM proposal `mg2044`, contributed to this suite | Contributed by the data owner for redistribution with this package |
