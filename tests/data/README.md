...
| `11BM_Si640c.xy` | NIST SRM 640c silicon powder pattern, APS beamline 11-BM run 4918 (25 Feb 2010), λ seeded at the header's stated 0.412359 Å, 48000 points, 1.996–49.995° 2θ at 0.001°, three columns (2θ, I, propagated σ). Header: `Sample name = Silicon (Si) NIST SRM 640c, certified cell length of 5.4311946 Angstroms` / `Run no. = 4918` / `Calibration file = feb10/11bmb_4917.calib` / `Calibrated wavelength = 0.412359`. The single-histogram refinable-wavelength anchor (`test_acceptance_si640c.py`) — see the section below | APS 11-BM beamline standard scan (run 4918, Feb 2010): a calibration measurement of a NIST SRM taken by the facility, not a user sample — its header carries no proposal, user or contact field, unlike this directory's mail-in patterns | Argonne/APS standard-reference scan (U.S. Government work; publicly distributed) |
...
standard scan — a facility calibration measurement of a NIST SRM, U.S. Government
work, publicly distributed. 48 000 points,
