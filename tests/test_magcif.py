...
def test_a_magcif_reads_into_wp1327s_model(tmp_path, key):
    ...
    verbatim, the BNS number and name as metadata, the parent k as a record on
    the magnetic block rather than as ``Phase.propagation_vector``, and the
    moment on the site the loop names.
    """
    spec = _ALL[key]
    ...
    # k is a *record* on the magnetic block (WP-1327's field), never
    # ``Phase.propagation_vector`` — declaring both of those on one phase is
    # refused (WP-1326), and a magCIF states k for provenance.
    assert ms.propagation_vector_parent == ("0", "0", "0")
    assert phase.propagation_vector is None
...
def test_an_anti_translation_inconsistent_moment_set_is_flagged_not_fixed(
        tmp_path):
    """D7, synthetic (not a real MAGNDATA entry): the reader validates each
    site's moment against *its own* site-symmetry subspace, but never checks
    the whole moment set against the group's other operations and
    centerings -- so two sites related by an anti-translation ({1 | t, -1})
    can each pass its own per-site check while together stating a moment set
    the declared group does not actually leave invariant.

    Per the brief: not fixed (choosing which site's statement is right is
    not a reader's call), only made loud with a diagnostic above a tolerance
    (1e-6 mu_B) sized well past round-off.
    """
    ops = ("x,y,z,+1",)
    cent = ("x,y,z,+1", "x+1/2,y,z,-1")  # an anti-translation along a

    def _fixture(*, fe2_z: str):
        return _magcif(
            tmp_path, "p1_anti", parent="A 1", it=1, bns="1.1", symbol="A 1",
            cell=(5.0, 6.0, 7.0, 90.0, 90.0, 90.0),
            operations=ops, centerings=cent,
            sites=("Fe1 Fe 0.00000 0.00000 0.00000 1",
                   "Fe2 Fe 0.50000 0.00000 0.00000 1"),
            moment_loop=_moment_loop([
                ("Fe1", "0.0", "0.0", "2.00000", "0,0,mz"),
                ("Fe2", "0.0", "0.0", fe2_z, "0,0,mz"),
            ]))

    # positive arm: Fe2 states the *same* sign as Fe1, but the anti-
    # translation relating them demands the opposite one -- residual 4 mu_B
    diagnostics: list = []
    structure_from_cif(str(_fixture(fe2_z="2.00000")),
                       moment_ions={"Fe1": "Fe3+", "Fe2": "Fe3+"},
                       diagnostics=diagnostics)
    hits = [d for d in diagnostics
            if d.code == "CIF_MAGNETIC_ANTI_TRANSLATION_RESIDUAL"]
    assert len(hits) == 1
    assert hits[0].level == "warning"
    assert hits[0].value == pytest.approx(4.0)
    assert "not invariant" in hits[0].message

    # negative control: Fe2 states the sign the anti-translation actually
    # demands -- the set is genuinely self-consistent and draws nothing
    diagnostics2: list = []
    structure_from_cif(str(_fixture(fe2_z="-2.00000")),
                       moment_ions={"Fe1": "Fe3+", "Fe2": "Fe3+"},
                       diagnostics=diagnostics2)
    assert not [d for d in diagnostics2
               if d.code == "CIF_MAGNETIC_ANTI_TRANSLATION_RESIDUAL"]
...
def test_an_ima2_type_origin_mismatch_validates_from_the_operators(tmp_path):
    """Bug B, synthetic. gemmi's tabulated operator table for a Hermann-Mauguin
    symbol can sit at a different origin than a magCIF's own magnetic
    ...
    Before the fix, ``structure_from_cif`` built this phase from the bare
    label ``"I m a 2"`` with no ``symmetry_operations`` at all, so
    :class:`~rietx.schemas.structure.Phase` ran no cross-check and the wrong
    (gemmi-canonical) coordinate frame was silently attached to a
    right-looking name — the failure mode is silent corruption, not a raise.
    The fix derives the operators from the file itself, so the disagreement
    becomes visible: the label is bracketed and a diagnostic is filed.
    """
    ...
    structure = structure_from_cif(str(path), moment_ions={"Fe1": "Fe3+"},
                                   diagnostics=diagnostics)
    (phase,) = structure.phases
    assert phase.space_group == "I m a 2 [unnamed in this cell]"
    assert phase.symmetry_operations is not None

    import gemmi

    from rietx.schemas.structure import _op_key

    tabulated = {_op_key(op)
                for op in gemmi.find_spacegroup_by_name("I m a 2").operations()}
    derived = {_op_key(gemmi.Op(o)) for o in phase.symmetry_operations}
    assert derived != tabulated, "the fixture must reproduce a real mismatch"
    assert derived == {_op_key(gemmi.Op(o)) for o in
                       magcif.nuclear_operations(phase.magnetic_symmetry)}
    ...
    assert "46" in hit.message and "I m a 2" in hit.message


def test_a_bracketed_magnetic_phase_survives_a_fit_and_a_whole_report(tmp_path):
    """Report crash, synthetic (the same Ima2 fixture as the test above, no
    MAGNDATA content). ``ref.report()`` crashed with ``ValueError: unknown
    space group symbol: 'I m a 2 [unnamed in this cell]'`` on any phase
    carrying this label -- i.e. on *every* WP-1328-fixed magCIF phase --
    whenever it also has a genuine magnetic-only reflection (a lattice point
    the nuclear glide/screw symmetry forbids but the magnetic group allows;
    Ima2's own (200)-type intensities are one).

    Located: not ``report.texture``'s ``resolve_group`` call itself (which
    already carries the frozen operation list correctly whenever one is
    there), but two call sites one rank down that silently dropped it while
    building a *merged* reflection set --
    :func:`~rietx.crystallography.magnetic.scattering.merge_magnetic` and
    :func:`~rietx.crystallography.satellites.merge_satellites` both rebuild a
    fresh ``ReflectionSet`` from the nuclear one's ``spacegroup`` but not its
    ``operations``, so a bracketed label survived the merge with no list left
    to resolve it from. Confirmed by tracing ``generate_reflections``: the
    freshly generated nuclear set carries ``operations`` correctly; the
    compiled phase's own ``reflections.operations`` was ``None`` only after
    ``merge_magnetic`` ran (i.e. only once a magnetic-only reflection existed
    to merge -- Ima2's k = 0 does, so this always fired for it).
    """
    ops = ("x,y,z,+1", "-x,-y,z,+1", "x,-y+1/2,z,-1", "-x,y+1/2,z,-1")
    cent = ("x,y,z,+1", "x+1/2,y+1/2,z+1/2,+1")
    path = _magcif(
        tmp_path, "ima2_report_synthetic", parent="I m a 2", it=46, bns="46.245",
        symbol="I m' a' 2", cell=(5.0, 15.0, 5.5, 90.0, 90.0, 90.0),
        operations=ops, centerings=cent,
        sites=("Fe1 Fe 0.00000 0.00000 0.00000 1",),
        moment_loop=_moment_loop([("Fe1", "0.0", "0.0", "2.00000", "0,0,mz")]),
    ...
    structure = structure_from_cif(str(path), moment_ions={"Fe1": "Fe3+"})
    assert "[unnamed in this cell]" in structure.phases[0].space_group

    instrument = rx.Instrument.constant_wavelength_neutron(wavelength=2.4)
    two_theta = np.arange(8.0, 90.0, 0.05)
    ...
    data = PatternData(two_theta=two_theta.tolist(), intensity=y.tolist())
    plan = rx.RefinementPlan(stages=[rx.Stage("all", ["phases.*.scale"],
                                              max_iter=5)])
    result = ref.fit(data, plan=plan)
    assert str(result.status) in ("converged", "max_iter")
    ...
    # and stage_reports=True: build_report() runs internally at every stage
    # boundary too (StageReport is a lighter *projection* with no texture
    # field of its own, per WP-1058 -- the crash under test happens building
    # it, not reading it back), so this raised the same ValueError before the
    # fix
    ref2 = rx.Refinement(structure.model_copy(deep=True),
                         instrument.model_copy(deep=True))
    ref2.fit(data, plan=plan, stage_reports=True)
    assert ref2.stage_reports_
