...
def test_a_phase_with_no_moment_is_bit_identical():
    ...
    assert cp.mag_members is None and cp.reflections.satellite_order is None
...
def test_the_capability_flag_is_derived_from_the_fields():
    ...
    # ``Atom.moment`` landed with WP-1327; WP-1329's ``SeriesEntry.magnetic``
    # and ``MomentEvidence.path`` follow it.
    # M-1 (distortion modes, folding in Q-17's Phase.symmetry_operations) and
    # WP-1343 (Phase.magnetic_lor_size / magnetic_lor_strain) both claimed the
    # same literal on separate branches; the integration merge conflicted as
    # the plan's rule for two branches moving one literal intends, and put M-1
    # first with WP-1343 after it. M2 follows for
    # ``Phase.distortion_components``, a grouped read-only view over
    # ``distortion_modes`` (no new stored field, still re-pinned). M-3 takes
    # the last two: Stage.distortion_seed's two mirrors, then
    # RefinementResult.distortion_totals and the DistortionTotal rows.
    # Cut onto main (the re-cut, 2026-09-18) renumbered the whole chain against
