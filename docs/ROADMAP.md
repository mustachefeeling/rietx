...
**[1121](wp/1121-per-reflection-cost.md) closed 2026-08-22 — the front is measured, named, and it is not another kernel**: the per-reflection blocks are **dispatch-bound**, which inverts 1115's reading of the planes — 98–282 reflections a phase, and a `phase_peaks` call in which *every* memo slot hits still costs 562 ns per reflection against 2–4 ns an element on the ~10⁵-element planes.  That is why the tier bought 11.0× on the scatter and **1.03×** here.  Two changes landed with unlike bars: the phase scale is exactly linear so its column is closed-form (**exact**, not bit-identical — the whole-model FD is the *wrong* oracle for a transformed parameter and would certify the column being replaced, now a root CLAUDE.md rule), and the scalar memo went two keys deep (**bit-identical**; depth 8 builds the same blocks as depth 2, to the call).  1.02–1.08× across the harness — trigger cold **8.70–8.72 s**.  **So the cold target is still missed, by ~3×, and two things remain — one of them a decision that is open.**  (a) **the flip is in**: [1123](wp/1123-fast-tolerance-default.md) closed 2026-08-22 on the maintainer's call, so intermediate stages stop at `ftol=1e-6` by default through one plan field (`RefinementPlan.intermediate_ftol`) and one authority (`stage_ftols`).  Measured on the merged tree: **trigger cold 8.63–8.65 → 5.67–5.70 s** (1121's estimate was ≈ 5.4), cpd-1a 408 → 270 nfev, cpd-2 533 → 329, nac 47 → 39, Rwp equal to five decimals, and the widest non-degenerate parameter move **0.027 esd**.  `intermediate_ftol=None` is the bit-identical way back and what a golden declares.  A **chained series changed sign** between two measurements one commit apart (1.04× worse, then 1.12× better), so the standing rule is that a series is measured rather than assumed.  (b) **[1122](wp/1122-compiled-peaks-buffer.md)**, the stretch (< 1 s cold): 1114's buffer design on 1115's substrate, the package's first *declared-tolerance* mode if it lands.  Its `### Inherited` carries 1121's remainder and one correction — the buffer attacks element *volume*, priced against the plane seam that is already ~2× off its floor, and buys nothing on the per-reflection 20 %.
...
| [1122](wp/1122-compiled-peaks-buffer.md) | Compiled peaks buffer: the declared-tolerance tier | ⬜ | 1115, 1121 (gate) |
