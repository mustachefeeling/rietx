...
**[1115](wp/1115-compiled-kernel-spike.md)'s gate is read 2026-08-22 — OPEN**, on neither mechanism it was written around: dispatch is gone and the ragged axis costs 1.11×, while 93 % of the trigger cold fit is peak-plane arithmetic and **71 % of that is now the Jacobian**, so the case is fusion and threading.  numba prototypes of both kernels measure 2.1–3.4× serial at a few ulp, up to 11× threaded where rows do not collide; its § Gate reading holds the tables, the projection and the two caveats on it.  **The `[speed]`-extra packaging decision is the user's and is what blocks**, then 1113's priced preset flip.
