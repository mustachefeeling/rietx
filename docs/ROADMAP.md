...
**v1.2 — the GUI for a crystallographer** ([record](milestones/v1.2.md)); the maintainer's use notes triaged into 17 WPs (1201-1217) plus [1017](wp/1017-gui-manual-onboarding.md), the manual, last. Eleven shipped: [1201](wp/1201-gui-house-style.md) (the token layer and nine registers), [1204](wp/1204-developer-mode-example-projects.md) (the empty state openable), [1202](wp/1202-help-corpus.md) (`src/rietx/help.py`, the corpus), [1203](wp/1203-help-popover.md) (one popover mechanism for explaining a name), [1205](wp/1205-new-project-open-browse-defaults.md) (`GET /api/fs`, so the empty state reaches any project), [1206](wp/1206-typed-cell-project.md) (**a project needs a lattice, not a structure**), [1207](wp/1207-pattern-only-project.md) (**a pattern is already a project** — the refusal moves to the verb, `NO_PHASES`), [1208](wp/1208-plan-introduction.md) (**a plan is a ladder and it says what it will free**), [1209](wp/1209-peaks-table-numbers-flags.md) (**the peak table reads like a peak table** — `esdSwallowsValue`, and a chip's words are the corpus's), [1210](wp/1210-peak-layer-identity.md) (**the peak layer stops borrowing** — its own `--plot-*` tokens, drawn only where it can be edited), [1211](wp/1211-candidate-overlay.md) (**a cell's claim, drawn over the pattern that has to answer it** — `GET /api/index/ticks?candidate=`, capped by rank in 2θ with `n_total` beside it, a preview and a selection two props) and now [1212](wp/1212-redraw-never-moves-axes.md) — **a redraw never moves the axes**. WP-1044's repair kept an axis only once `autorange === false`, which plotly writes on a zoom and nowhere else, so on the plot nobody had zoomed every redraw and every `restyle` re-fitted them; the axes are now explicit after the paint that fitted them, and hover, tab change, arming and a whole exclude drag (4 reacts → 1) move nothing. Three rules reach past the panel. **The range plotly is drawing with is `ax._rl`, not `ax.range`** — on a fresh plot `range` was still the empty-axis default while the ticks and the pixel map said 0-60°, and pinning it froze the raw view blank. **A layout key is a `relayout`, never a repaint**, and a `$derived` off `project` arrives new-but-equal on every settings PATCH, so an effect keyed on the object repaints for nothing. And **an empty `scattergl` trace gets no index in the scene its peers share**, so a select drag read `selectBatch[undefined]` and threw once per pointer move. What v1.2 does not cover yet: 47 authored tooltips remain elsewhere, per file, counted since 1203 (`rietx.help` has no arm for their vocabularies yet); and the residual subplot's empty quarter, which 1212 measured and decided against collapsing (the x axis is anchored to `y2` and the tick band owns the gap, so it is a layout redesign). **Next: [1213](wp/1213-hover-readout.md)** (the hover readout — the `hkl` and `line` arms 1211 serves and deliberately does not draw, in a strip that does not cover the data), then Model (1214-1216), History (1217) — each of which inherits its panel's tooltip count.
