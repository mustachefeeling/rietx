...
**v1.2 — the GUI for a crystallographer** ([record](milestones/v1.2.md)); the maintainer's use notes triaged into 17 WPs (1201-1217) plus [1017](wp/1017-gui-manual-onboarding.md), the manual, last. Eight shipped: [1201](wp/1201-gui-house-style.md) (the token layer and nine registers), [1204](wp/1204-developer-mode-example-projects.md) (the empty state openable), [1202](wp/1202-help-corpus.md) (`src/rietx/help.py`, the corpus), [1203](wp/1203-help-popover.md) (one popover mechanism for explaining a name), [1205](wp/1205-new-project-open-browse-defaults.md) (`GET /api/fs`, so the empty state reaches any project), [1206](wp/1206-typed-cell-project.md) (**a project needs a lattice, not a structure**), [1207](wp/1207-pattern-only-project.md) (**a pattern is already a project** — `structure=None` is legal and the refusal moves to the verb, `NO_PHASES`), [1208](wp/1208-plan-introduction.md) (**a plan is a ladder and it says what it will free**, `GET /api/plan/resolve` running `set_vary` itself) and now [1209](wp/1209-peaks-table-numbers-flags.md) — **the peak table reads like a peak table**: intensities on a 0-100 scale over the strongest *measured* line with `—` where the number is not one, 2θ at four places with the esd in the last place, `use` as its own column, and every flag a two-word corpus label with its code behind the popover. Two rules reach past the panel: **an esd that has swallowed its value is not a precision** (`esdSwallowsValue`, so `35.09 ±110` replaces `35(111)` in every table, `12346(56)` unchanged), and **a chip's words are the corpus's** (`HelpEntry.label`, the second authored field). One measurement overturned the triage: the 0.1° esd threshold hid a real line's esd on the first pattern it met, and the boundary is a degree, wider than any peak. What v1.2 does not cover yet: 47 authored tooltips remain elsewhere, per file, counted since 1203 (`rietx.help` has no arm for their vocabularies yet). **Next: [1210](wp/1210-peak-layer-identity.md)** (the peak layer), then the rest of the plot (1211-1213), Model (1214-1216), History (1217) — each of which inherits its panel's tooltip count.
