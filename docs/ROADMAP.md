...
**[1115](wp/1115-compiled-kernel-spike.md)'s gate is read 2026-08-22 — OPEN**, on neither mechanism it was written around: dispatch is gone and the ragged axis costs 1.11×, while 93 % of the trigger cold fit is peak-plane arithmetic and **71 % of that is now the Jacobian**, so the case is fusion and threading.  numba prototypes of all three kernels — forward 2.1–2.4×, bases 3.2–3.4× (11× threaded), the column scatter 6.9× and *bit-identical* — cover 63 % of the fit and project it 17.1 → **8.0 s**, ≈ 5 s with 1113's priced preset flip: the warm-series target reached, the cold one still missed ~2×.  The column seam is only 57 % plane work, which is what took the projection down from 3.2× to 2.15×, and the front after a compiled tier is the 15 608 perturbed `phase_peaks` a Jacobian asks for.  **The `[speed]`-extra packaging decision is the user's and is what blocks.**
