...
**v1.2 — the GUI for a crystallographer** ([record](milestones/v1.2.md)); the maintainer's use notes triaged into 17 WPs (1201-1217) plus [1017](wp/1017-gui-manual-onboarding.md), the manual, last. Five shipped: [1201](wp/1201-gui-house-style.md) (the token layer and nine registers), [1204](wp/1204-developer-mode-example-projects.md) (the empty state openable), [1202](wp/1202-help-corpus.md) (`src/rietx/help.py`, the corpus), [1203](wp/1203-help-popover.md) (one popover mechanism for explaining a name) and now [1205](wp/1205-new-project-open-browse-defaults.md) — **the empty state can reach any project, not only the ones this build already knows about**: `GET /api/fs` browses the home directory and cwd, and a real structural bug (`Model` mounted twice, so an opened project's wizard sometimes stuck) is fixed rather than patched. What it does not cover: 51 authored tooltips remain elsewhere, per file, counted since 1203 (`rietx.help` has no arm for their vocabularies yet). **Next: [1206](wp/1206-typed-cell-project.md)-[1207](wp/1207-pattern-only-project.md)** (a project without a CIF), then Plan (1208), Peaks and the plot (1209-1213), Model (1214-1216), History (1217) — each of which inherits its panel's tooltip count.
