...
**v1.3 shipped 2026-08-30** ([record](milestones/v1.3.md), [notes](releases/1.3.0.md)): the surface a *program* drives this package through, refactored against measured runs rather than intuition. Eight WPs, 1301-1308, each with its narrative in the record; the six acceptance rows are in the milestone table below and in the record, and they were written **at ship rather than at the open**, which the record says plainly is the weaker evidence. Five rules outlived their WPs. **A dedicated tool surface earns its place only where it gates, renders, audits or parallelises** — none of which a shell-equipped agent needs, which is why `rietx.agent` is deleted rather than deprecated and there is one integration surface. **A companion rides beside an answer, never inside it**, and **a value that is not a measurement is the caller's**: a held parameter leaves `RefinementResult.parameters`, so a trajectory starts at the onset instead of opening with cells nobody measured. **A lookup leaves the body, a rule and its decisive number stay** — the cap that shaped the skill, and the reason its API index is generated rather than typed. And **two references are an envelope, not a second tolerance**: where a fixture carries two engines they disagree by 2665 ppm on a cell whose band is 300, so the gate is the span they bracket. What v1.3 does not cover: round 1.2 owes a **sealed workspace** before any `bare` cell is read as an access claim; `tests/CLAUDE.md` is owed two rules and sits at exactly its 253-line cap, and CLAUDE.md's skill bullet still states only the half WP-1306 followed (written, measured, reverted — the derived gate carries it instead); and `.inp`/`.EXP`/`.pcr` still have no reader, which the skill now says out loud ([1118](wp/1118-foreign-model-files.md), unscheduled). **Next: v1.4 — free-standing peaks** (`fit_peaks` + the extra-components seam), 1101-1103, which have kept their numbers across two slips.

**v1.2 shipped 2026-08-28** ([record](milestones/v1.2.md), [notes](releases/1.2.0.md)): the GUI documented, out of beta, and no longer a thing a first-time crystallographer has to be taught around. Its durable rules: **a vocabulary a document is *about* can be partitioned even when its prose cannot be checked** (77 routes, nine panels, tightening both ways); **where no python object knows the fact, the authorities swap** (the tab strip is data in `gui/src/lib/tabs.ts`, vitest writes the corpus and pytest reads it); **a screenshot is generated or it is a lie with a timestamp**; and **a count in prose rots exactly like a retuned threshold**. Still uncovered: 47 authored tooltips in eleven files, each owed a corpus arm, and the phase's corrections, which no form offers because offering one is offering to declare it.
...
Parked, none of it blocking: the 1.0.0-release-notes promises (`.rex` zip transport, and `RefinementState.excluded_regions` with `replay` honouring them, 1003 §B); the post-1003 indexing work (narrow the acceptance fixtures' search, and the `grade` prior-counting change, 1046 §4); the model-cost estimate (1110's ask, priced in 1113 § Findings); the two v1.1 speed fronts still unowned (the per-reflection 19.4 %, and the `refit=` choice that discards 50 % of a trigger series' wall in ladder rungs); [1118](wp/1118-foreign-model-files.md), [1119](wp/1119-named-variables.md), [1130](wp/1130-background-reference.md), [1131](wp/1131-sample-broadening-is-a-specimen-property.md) and [1133](wp/1133-diagnostic-names-its-view.md), unscheduled — 1130 reviewed 2026-08-27 and restructured behind 1131; and the LaB6+cBN correlation test's **Linux-only red**, which has never had a green run there and is a disagreement about which parameters a degenerate fit's guard names ([v1.3 record](milestones/v1.3.md) § Appendix — acceptance at ship).
...
| [1308](wp/1308-skill-documents-its-doors.md) | The skill documents its own doors | ✅ | 1304, 1306, 1307 |
...
([DESIGN.md](DESIGN.md#locked-decisions)), and pre-writing packages invites scope creep.
