...
**v1.2 — the GUI for a crystallographer** ([record](milestones/v1.2.md)); the maintainer's use notes triaged into 17 WPs (1201-1217) plus [1017](wp/1017-gui-manual-onboarding.md), the manual, last. Sixteen shipped — [1201](wp/1201-gui-house-style.md) … [1215](wp/1215-structure-table.md), one line each in the record — and now [1216](wp/1216-instrument-form.md): **the instrument section is a form rather than a list**. Source, Geometry and Profile, in the order the quantities are decided in, with U V W directly above X Y. Three rules reach past the panel. **Which group a field is in is data, never its path's prefix** — the axial apertures shape the peak and are drawn with the profile, the zero shift belongs beside the displacement it is refined against — and a group no field is in would be a heading over nothing, which is why the background is not one. **A form's column count is a promise, so nothing else may decide it**: a wrapping row lets its widest item pick the break (one `<select>` moved every field after it) and an auto-fill count lets the container pick it (five columns of Source against three of Profile at the 559 px ceiling), so the grid is fixed at three — the profile's own row — and a control whose content is a word takes the row rather than a track. And **a declared column count is a declared minimum**: `MODEL_MIN.form` became 3 x 92 + 2 x 10 + 24 = 320 and the pane stacks below 1256 px rather than 1136, the number stated once and crossed by a test against the panel's own CSS constants — WP-1215's stale width, caught by machine this time. What v1.2 does not cover yet: 47 authored tooltips elsewhere, per file, counted since 1203; the residual subplot's empty quarter, which 1212 measured and decided against collapsing; and the phase's corrections (extinction, preferred orientation, Stephens), which no form offers because offering one is offering to declare it. **Next: [1217](wp/1217-history-graph-compare.md)** (the history graph and the compare table — the last of the triage; it inherits the 1256 px threshold), then [1017](wp/1017-gui-manual-onboarding.md).
