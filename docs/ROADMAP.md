...
**v1.2 — the GUI for a crystallographer** ([record](milestones/v1.2.md)); the maintainer's use notes triaged into 17 WPs (1201-1217) plus [1017](wp/1017-gui-manual-onboarding.md), the manual, last. Eight shipped: [1201](wp/1201-gui-house-style.md) (the token layer and nine registers), [1204](wp/1204-developer-mode-example-projects.md) (the empty state openable), [1202](wp/1202-help-corpus.md) (`src/rietx/help.py`, the corpus), [1203](wp/1203-help-popover.md) (one popover mechanism for explaining a name), [1205](wp/1205-new-project-open-browse-defaults.md) (`GET /api/fs`, so the empty state reaches any project), [1206](wp/1206-typed-cell-project.md) (**a project needs a lattice, not a structure**), [1207](wp/1207-pattern-only-project.md) (**a pattern is already a project** — `structure=None` is legal and the refusal moves to the verb, `NO_PHASES`) and now [1208](wp/1208-plan-introduction.md) — **a plan is a ladder and it says what it will free**: `GET /api/plan/resolve` runs `ParameterTable.set_vary` itself over the table `fit` builds, so each stage shows the parameters it adds on top of the last, the ones its globs reach and cannot free with the row's own reason, and the Rwp of the node that ran it. Two findings to carry forward: **a fit replaces the vary flags a person set while a single stage continues them** (measured, and `set_aside` is the panel naming it), and a `fit(stage_reports=True)` trajectory costs 7.7× the fit for Rwps the history nodes already carry bit-identically. What v1.2 does not cover yet: 49 authored tooltips remain elsewhere, per file, counted since 1203 (`rietx.help` has no arm for their vocabularies yet). **Next: [1209](wp/1209-peaks-table-numbers-flags.md)** (the peaks table), then the plot (1210-1213), Model (1214-1216), History (1217) — each of which inherits its panel's tooltip count.
