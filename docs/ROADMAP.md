...
**[1121](wp/1121-per-reflection-cost.md) closed 2026-08-22 — the front is measured, named, and it is not another kernel**: the per-reflection blocks are **dispatch-bound**, which inverts 1115's reading of the planes — 98–282 reflections a phase, and a `phase_peaks` call in which *every* memo slot hits still costs 562 ns per reflection against 2–4 ns an element on the ~10⁵-element planes.  That is why the tier bought 11.0× on the scatter and **1.03×** here.  Two changes landed with unlike bars: the phase scale is exactly linear so its column is closed-form (**exact**, not bit-identical — the whole-model FD is the *wrong* oracle for a transformed parameter and would certify the column being replaced, now a root CLAUDE.md rule), and the scalar memo went two keys deep (**bit-identical**; depth 8 builds the same blocks as depth 2, to the call).  1.02–1.08× across the harness — trigger cold **8.70–8.72 s**.  **So the cold target is still missed, by ~3×, and two things remain — one of them a decision that is open.**  (a) **1113's preset flip**: intermediate stages at `ftol=1e-6` buy **1.5–1.7× fewer whole-plan evaluations at ≤ 0.02 esd** of parameter movement (priced in [1113](wp/1113-evaluation-count.md) § Findings, presets never flipped).  Against 8.7 s that is ≈ 5.4 s, it is the larger remaining factor, and it is the maintainer's call — recorded here because 1121 exists precisely to repair a forward finding that was orphaned.  (b) **[1122](wp/1122-compiled-peaks-buffer.md)**, the stretch (< 1 s cold): 1114's buffer design on 1115's substrate, the package's first *declared-tolerance* mode if it lands.  Its `### Inherited` carries 1121's remainder and one correction — the buffer attacks element *volume*, priced against the plane seam that is already ~2× off its floor, and buys nothing on the per-reflection 20 %.
