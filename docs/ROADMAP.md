...
**v1.3 — agents and programs** is in flight ([record](milestones/v1.3.md), 1301-1307 filed 2026-08-28): the agent surface refactored against two measured runs rather than intuition. It opened 2026-08-28 with the version bump to `1.3.0.dev0`, and [1301](wp/1301-hold-unsupported-phase.md) shipped the same day — a phase the data cannot see is **held for the stage** rather than bounded, its scale left free so it can still appear, and support is re-measured at the answer because a stage moves the values: a phase that appeared is released and the stage re-solved, one that collapsed is put back where the stage found it. What a caller gets for such a phase is the value they supplied, not where a walk stopped. **Next: [1302](wp/1302-error-is-documentation.md)**, the termination view. Free-standing peaks (1101-1103) stay at **v1.4**.
