...
**v1.2 — the GUI for a crystallographer** ([record](milestones/v1.2.md)); the maintainer's use notes triaged into 17 WPs (1201-1217) plus [1017](wp/1017-gui-manual-onboarding.md), the manual, last. Nine shipped: [1201](wp/1201-gui-house-style.md) (the token layer and nine registers), [1204](wp/1204-developer-mode-example-projects.md) (the empty state openable), [1202](wp/1202-help-corpus.md) (`src/rietx/help.py`, the corpus), [1203](wp/1203-help-popover.md) (one popover mechanism for explaining a name), [1205](wp/1205-new-project-open-browse-defaults.md) (`GET /api/fs`, so the empty state reaches any project), [1206](wp/1206-typed-cell-project.md) (**a project needs a lattice, not a structure**), [1207](wp/1207-pattern-only-project.md) (**a pattern is already a project** — the refusal moves to the verb, `NO_PHASES`), [1208](wp/1208-plan-introduction.md) (**a plan is a ladder and it says what it will free**), [1209](wp/1209-peaks-table-numbers-flags.md) (**the peak table reads like a peak table** — `esdSwallowsValue`, and a chip's words are the corpus's), [1210](wp/1210-peak-layer-identity.md) (**the peak layer stops borrowing** — `--plot-peak`/`--plot-peakfit` in place of chrome that *is* `--plot-diff`/`--plot-calc` on the light theme, the layer drawn only where it can be edited, `data only` restoring the picture rather than everything) and now [1211](wp/1211-candidate-overlay.md) — **a cell's claim, drawn over the pattern that has to answer it**: `GET /api/index/ticks?candidate=` returns one candidate's predicted positions and selecting its row draws them full height through the data, so `predicted_but_absent` stops being only a count. Three rules reach past the panel. **A plot mark takes a `--plot-*` token of its own** (WP-1210; `tests/test_gui_palette.py` holds every one to a 0.13 OKLab floor and names the two pairs the shipped set already misses) — and with `--plot-candidate` green the free hue space is now **spent**, so the next mark that carries a quantity has to find its separation in the mark. **A cap says that it capped**: 2000 lines, thinned by rank in 2θ with `n_total` beside it, because `max_d_axis` admits a cell predicting 92 103 over a lab range and a head-of-list cut would leave the high-angle half reading as "nothing predicted here". And **a preview is not a selection** — a hover draws, only a selection clears the plot — which is two props, because one made the model strobe once per row the pointer crossed. What v1.2 does not cover yet: 47 authored tooltips remain elsewhere, per file, counted since 1203 (`rietx.help` has no arm for their vocabularies yet). **Next: [1212](wp/1212-redraw-never-moves-axes.md)** (a redraw never moves the axes, and it already owns 1210's finding that a hidden residual keeps its subplot's domain), then [1213](wp/1213-hover-readout.md) (the hover readout — the `hkl` and `line` arms 1211 serves and deliberately does not draw), Model (1214-1216), History (1217) — each of which inherits its panel's tooltip count.
