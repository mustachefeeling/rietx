...
**[1115](wp/1115-compiled-kernel-spike.md) shipped its tier 2026-08-22 — and it is the default install**: the gate read open on neither mechanism the WP was written around (dispatch is gone, the ragged axis costs 1.11×) but on **fusion and threading**, so `model/compiled.py` fuses the three kernels over `nogil` + a shared thread pool — trigger **17.6 → 8.9 s**, cpd-1a **4.2 → 2.2 s**, nac 0.54 → 0.40 s, Rwp equal to five decimals in all four.  The scatter is *bit-identical*; symmetric rows are too, in window, once each Ω spelling is transcribed faithfully (the 1120 difference is one association, and numba's `exp` matches numpy's bit for bit here); FCJ rows keep 1112's rounding bar.  **numba is a core dependency**, the user's call against the priced +157 MB and `numpy<2.6`: an extra cannot subtract a dependency, so the way out is a soft import plus `RIETX_COMPILED=0`, both reported by `capabilities().features`.  Cold JIT 0.60 s, 0.12 s cached, background-warmed from `compile_model`.  The cold target is still missed ~2×, and the front is now the 15 608 perturbed `phase_peaks` a Jacobian asks for.
