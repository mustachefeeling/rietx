...
**v1.2 — the GUI for a crystallographer** ([record](milestones/v1.2.md)); the maintainer's use notes triaged into 17 WPs (1201-1217) plus [1017](wp/1017-gui-manual-onboarding.md), the manual, last. Four shipped: [1201](wp/1201-gui-house-style.md) (the token layer and nine registers), [1204](wp/1204-developer-mode-example-projects.md) (the empty state openable), [1202](wp/1202-help-corpus.md) (`src/rietx/help.py`, the corpus) and now [1203](wp/1203-help-popover.md) — **the GUI has one mechanism for explaining a name**: a corpus entry reached through `<Help>`, keyboard-reachable, with the manual one click away. What it does not yet cover is *counted*, not hidden: 51 authored tooltips remain, per file, because they name vocabularies `rietx.help` has no arm for. **Next: [1205](wp/1205-new-project-open-browse-defaults.md)**, the first WP that *uses* the popover rather than building it; its `### Inherited` carries what 1203 and 1204 left it. Then the § v1.2 table's order: 1206-1207 (a project without a CIF), Plan (1208), Peaks and the plot (1209-1213), Model (1214-1216), History (1217) — each of which inherits its panel's tooltip count.
