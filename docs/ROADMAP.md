...
**v1.2 — the GUI for a crystallographer** ([record](milestones/v1.2.md)); the maintainer's use notes triaged into 17 WPs (1201-1217) plus [1017](wp/1017-gui-manual-onboarding.md), the manual, last. Nine shipped: [1201](wp/1201-gui-house-style.md) (the token layer and nine registers), [1204](wp/1204-developer-mode-example-projects.md) (the empty state openable), [1202](wp/1202-help-corpus.md) (`src/rietx/help.py`, the corpus), [1203](wp/1203-help-popover.md) (one popover mechanism for explaining a name), [1205](wp/1205-new-project-open-browse-defaults.md) (`GET /api/fs`, so the empty state reaches any project), [1206](wp/1206-typed-cell-project.md) (**a project needs a lattice, not a structure**), [1207](wp/1207-pattern-only-project.md) (**a pattern is already a project** — the refusal moves to the verb, `NO_PHASES`), [1208](wp/1208-plan-introduction.md) (**a plan is a ladder and it says what it will free**), [1209](wp/1209-peaks-table-numbers-flags.md) (**the peak table reads like a peak table** — `esdSwallowsValue`, and a chip's words are the corpus's) and now [1210](wp/1210-peak-layer-identity.md) — **the peak layer stops borrowing**: `--plot-peak`/`--plot-peakfit` in place of `--accent`/`--bad`, which on the light theme *are* `--plot-diff`/`--plot-calc` to the digit, which is why the picked-peak fit and the model were one red line; the layer drawn only on the tab that can edit it, its toggles listed and disabled with the reason elsewhere, and a `data only` button that restores the picture rather than everything. Two rules reach past the panel: **a plot mark takes a `--plot-*` token of its own** (chrome is not a palette; `tests/test_gui_palette.py` is the 0.13 OKLab floor, and it names the two pairs the shipped set already misses rather than exempting them), and **state rides on the mark, never on a second colour** — which is now the cheap answer as well as the honest one, because the free hue space is nearly spent: violet is 0.10-0.12 from `--plot-diff`, `--warn` 0.053 from `--plot-calc`, magenta is taken, green is what is left. What v1.2 does not cover yet: 47 authored tooltips remain elsewhere, per file, counted since 1203 (`rietx.help` has no arm for their vocabularies yet). **Next: [1211](wp/1211-candidate-overlay.md)** (indexing candidates on the plot), then 1212-1213 (the axes, the hover readout), Model (1214-1216), History (1217) — each of which inherits its panel's tooltip count.
