...
**[1120](wp/1120-batched-residual.md) closed 2026-08-22**: the numpy forward runs on 1112's batched planes — trigger cold **17.1 s against 28.3** (1.65×), the four lab/synchrotron cases 1.11–1.15×, the whole gain inside `evaluate`.  Bit-identical wherever the rows are symmetric (four cases: identical per-stage nfev *and* Rwp), one ulp where they carry FCJ, which moves the trigger's count by one and no parameter or esd.
**[1115](wp/1115-compiled-kernel-spike.md) shipped its tier 2026-08-22 — and it is the default install**: the gate read open on neither mechanism the WP was written around (dispatch is gone, the ragged axis costs 1.11×) but on **fusion and threading**, so `model/compiled.py` fuses the three kernels over `nogil` + a shared thread pool — trigger **17.6 → 8.9 s**, cpd-1a **4.2 → 2.2 s**, nac 0.54 → 0.40 s, Rwp equal to five decimals in all four.  The scatter is *bit-identical*; symmetric rows are too, in window, once each Ω spelling is transcribed faithfully (the 1120 difference is one association, and numba's `exp` matches numpy's bit for bit here); FCJ rows keep 1112's rounding bar.  **numba is a core dependency**, the user's call against the priced +157 MB and `numpy<2.6`: an extra cannot subtract a dependency, so the way out is a soft import plus `RIETX_COMPILED=0`, both reported by `capabilities().features`.  Cold JIT 0.60 s, 0.12 s cached, background-warmed from `compile_model`.  The cold target is still missed ~2×, and the front is now the 15 608 perturbed `phase_peaks` a Jacobian asks for — per-reflection scalars no plane kernel reaches, which is **[1121](wp/1121-per-reflection-cost.md)**, opened 2026-08-22 and the next thing to pick up.  It multiplies with the other half, **[1123](wp/1123-fast-tolerance-default.md) closed 2026-08-22**: 1113's priced flip is the default, through one plan field (`RefinementPlan.intermediate_ftol`, 1e-6) and one authority (`stage_ftols`) — **272/315/232 evaluations against 408/540/358** on cpd-1a/cpd-2/trigger, 1113's counts to the evaluation, for 0.001/0.020/0.001 esd and QPA within 0.003 wt %.  `intermediate_ftol=None` is the bit-identical way back and what a golden declares.  The **chained series is the one case that does not pay** (1705 evaluations against 1634 — the chain turns a seed change into a rung escalation), documented rather than tuned; and `SCHEMA_VERSION` 0.5 is its first entry whose *default* moves an answer.  The stretch (< 1 s cold) has its one measured route owned too: **[1122](wp/1122-compiled-peaks-buffer.md)** — 1114's buffer design on 1115's substrate, the package's first *declared-tolerance* mode if it lands — gated on 1121's closing measurement.
...
| [1121](wp/1121-per-reflection-cost.md) | The per-reflection front: what a compiled tier does not reach | ⬜ | 1115 |
