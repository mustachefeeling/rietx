...
**v1.2 — the GUI for a crystallographer** ([record](milestones/v1.2.md)); the maintainer's use notes triaged into 17 WPs (1201-1217) plus [1017](wp/1017-gui-manual-onboarding.md), the manual, last. Twelve shipped — [1201](wp/1201-gui-house-style.md) (the token layer and nine registers), [1202](wp/1202-help-corpus.md) (`src/rietx/help.py`, the corpus), [1203](wp/1203-help-popover.md) (one popover mechanism), [1204](wp/1204-developer-mode-example-projects.md) and [1205](wp/1205-new-project-open-browse-defaults.md) (the empty state openable, and reaching any project), [1206](wp/1206-typed-cell-project.md) (**a project needs a lattice, not a structure**), [1207](wp/1207-pattern-only-project.md) (**a pattern is already a project**), [1208](wp/1208-plan-introduction.md) (**a plan is a ladder and it says what it will free**), [1209](wp/1209-peaks-table-numbers-flags.md) (**the peak table reads like a peak table**), [1210](wp/1210-peak-layer-identity.md) (**the peak layer stops borrowing**), [1211](wp/1211-candidate-overlay.md) (**a cell's claim, drawn over the pattern that has to answer it**) and [1212](wp/1212-redraw-never-moves-axes.md) (**a redraw never moves the axes**) — and now [1213](wp/1213-hover-readout.md): **the tooltip stops covering the data**. plotly offers no positioning for its unified box beyond `hoverlabel.align`, so the box is deleted rather than moved — every trace at `hoverinfo: "none"` (the library's gate is `!== "skip"`, so the point-finding and a spike survive), and a strip of labelled numbers under the plot saying what the box said plus the three things it could not: the candidate's `hkl`, which emission line it is, and d. Four rules reach past the panel. **The strip's shape follows the payload, the tab and the curve toggles — never the pointer**, so every field keeps its slot and is `ch`-sized; measured, the plot held 776 px and the strip 23 px across ten hover positions. **A curve is read at the nearest drawn channel and a nearby thing is hit-tested against the pointer** — the drawn pattern is decimated, so at a survey view the channel is up to ~0.03° away and the pointer sat on three picked lines in a row while the row read `—`. **The spike is chrome, so it is solid and takes `--fg`**: dotted in `--muted` is `maskShapes`' excluded-region edge exactly. And **prose takes `−`, numbers take `-`**, which the app followed unwritten until two spellings of the same sign landed in one row. What v1.2 does not cover yet: 47 authored tooltips remain elsewhere, per file, counted since 1203 (`rietx.help` has no arm for their vocabularies yet); and the residual subplot's empty quarter, which 1212 measured and decided against collapsing and 1213 left standing. **Next: [1214](wp/1214-model-vary-and-profile-save.md)** (Model — vary and profile save), then [1215](wp/1215-structure-table.md), [1216](wp/1216-instrument-form.md), [1217](wp/1217-history-graph-compare.md) — each of which inherits its panel's tooltip count.
