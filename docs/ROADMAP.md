...
**The plane seam finished 2026-08-22, in two closes** (narratives in the record).  **[1120](wp/1120-batched-residual.md)**: the numpy forward runs on 1112's batched planes — trigger cold **17.1 s against 28.3** (1.65×), the four lab/synchrotron cases 1.11–1.15×, the whole gain inside `evaluate`; bit-identical wherever the rows are symmetric (identical per-stage nfev *and* Rwp on four cases), one ulp where they carry FCJ, moving the trigger's count by one and no parameter or esd.  **[1115](wp/1115-compiled-kernel-spike.md) shipped its tier — and it is the default install**: the gate opened on neither mechanism the WP was written around (dispatch was gone, the ragged axis costs 1.11×) but on **fusion and threading**, so `model/compiled.py` fuses the three kernels over `nogil` + a shared thread pool — trigger **17.6 → 8.9 s**, cpd-1a **4.2 → 2.2 s**, nac 0.54 → 0.40 s, Rwp equal to five decimals in all four.  The scatter is *bit-identical*; symmetric rows are too, in window, once each Ω spelling is transcribed faithfully (numba's `exp` matches numpy's bit for bit here); FCJ rows keep 1112's rounding bar.  **numba is a core dependency**, the user's call against the priced +157 MB and `numpy<2.6`: an extra cannot subtract a dependency, so the way out is a soft import plus `RIETX_COMPILED=0`, both reported by `capabilities().features`.  Cold JIT 0.60 s, 0.12 s cached, background-warmed from `compile_model`.
**[1121](wp/1121-per-reflection-cost.md) closed 2026-08-22 — the front is measured, named, and it is not another kernel**: the per-reflection blocks are **dispatch-bound**, which inverts 1115's reading of the planes — 98–282 reflections a phase, and a `phase_peaks` call in which *every* memo slot hits still costs 562 ns per reflection against 2–4 ns an element on the ~10⁵-element planes.  That is why the tier bought 11.0× on the scatter and **1.03×** here.  Two changes landed with unlike bars: the phase scale is exactly linear so its column is closed-form (**exact**, not bit-identical — the whole-model FD is the *wrong* oracle for a transformed parameter and would certify the column being replaced, now a root CLAUDE.md rule), and the scalar memo went two keys deep (**bit-identical**; depth 8 builds the same blocks as depth 2, to the call).  1.02–1.08× across the harness — trigger cold **8.70–8.72 s**.  **Both of the things that remained are now decided.**  (a) **the flip is in**: [1123](wp/1123-fast-tolerance-default.md) closed 2026-08-22 on the maintainer's call, so intermediate stages stop at `ftol=1e-6` by default through one plan field (`RefinementPlan.intermediate_ftol`) and one authority (`stage_ftols`).  Measured on the merged tree: **trigger cold 8.63–8.65 → 5.67–5.70 s** (1121's estimate was ≈ 5.4), cpd-1a 408 → 270 nfev, cpd-2 533 → 329, nac 47 → 39, Rwp equal to five decimals, and the widest non-degenerate parameter move **0.027 esd**.  `intermediate_ftol=None` is the bit-identical way back and what a golden declares.  A **chained series changed sign** between two measurements one commit apart (1.04× worse, then 1.12× better), so the standing rule is that a series is measured rather than assumed.  (b) **[1122](wp/1122-compiled-peaks-buffer.md) closed 2026-08-22 — 🛑, and the stretch with it**: on 1115's substrate the buffer *does* pay where peaks are FCJ-smeared (**1.9-2.8×** on the plane seam, agreeing with 1114's prototype to 4.3e-16), but the plane seam is **54.9 %** of the cold fit, so a *free* one still leaves 2.56 s and < 1 s was never reachable by this route.  What decides shape reuse is neither the tolerance nor the anchor count but **images per window point**, break-even **2.78-4.16**: the trigger's phases carry 7.8 and clear it, every other harness family carries exactly 1.00 and loses 3-7×, so per-family selection leaves **1.48×** on one case of four.  Deferred, with a trigger rather than to a shelf: it ships **with FPA**, which is what raises that multiplier (root CLAUDE.md § Backends).  **v1.1's two gating targets are untouched by this close; its stretch is now recorded measured-unreachable rather than open.**  What is left of the milestone is the **warm-series band**, and [1124](wp/1124-warm-series-continuation.md) **closed 2026-08-22 — ✅ on a clean negative that moved the front rather than the number** (narrative in the record).  Predictor seeding (the survey's B8) bought at best **1.25×** fewer whole-chain evaluations for **no wall-clock change at all**, cost 1.16-1.67× *more* under `refit="stages"`, once broke the chain silently through a step its own corrector check passed (186 esd on a cell), and turned `SEQUENTIAL_PATH_DEPENDENT` from absent to reported in **both** arms — the kill criterion's clause — because a first-order predictor cannot seed the first two patterns of a pass and is asymmetric in the series coordinate by construction.  Nothing shipped; both arms were built on the existing `fit(prepare=…)` seam.  **The yield is the decomposition.**  The harness had been reporting the cheap half of the expensive patterns — one `fit_start`/`fit_end` pair per ladder *rung*, collector kept the last — so pattern 1 read 3.21 s and had spent 20.19 s: **57 % of the series wall is first rungs the ladder then discards**, seven of the nine warm patterns already sit at **0.89-2.32 s** against the ~1 s target, and `refit="stages"` runs the same chain **1.61×** faster (35.76-35.97 s, zero escalations) than the shipped `refit="single"` default.  So the band is **which rung the ladder starts on**, not what it starts from — unowned, and adaptive rather than a flipped default, since WP-0505 measured the opposite on small-cell standards.  **[1125](wp/1125-varpro-probe.md) closed 2026-08-22 — ✅, and the survey's A1 with it** (narrative in the record): profiling the background out (variable projection) cannot collapse 1113's count mechanism, because for an **unconstrained** linear block the profiled Gauss-Newton step **is** the joint one — the Schur complement, exact wherever the block sits, since (I − P)·M = 0 — so VarPro asks the solver for the step it already takes.  Measured ≤ 6.6e-07 on 70 stages; **34 of 34** stages where TRF took a pure Gauss-Newton step came back at the identical accepted-step count, every `zero_disp`/`cell` row is **1.00×** at both schedules with the decay ratio equal to three decimals, and 1113's baselines (84/86/93/131) reproduce exactly.  Whole probe **0.79×** — *worse*, because TRF's initial trust radius is ‖x0‖ and the background coefficients are counts carrying nearly all of it (shrink up to 59×).  Nothing shipped, `src/` untouched.  **The yield is a correction to the survey's reasoning**: Ruhe & Wedin's quadratic-vs-linear argument is against *alternation*, which this package has never done — now a root CLAUDE.md rule.  E5 stands on correctness (esds marginalised over the block) and Pawley **dimension**, both unmeasured, and neither may quote this result, which lapses the moment a bound goes active.  Both probes came from the survey's dated re-assessment ([solver-survey.md](solver-survey.md) §5) and **both were retired within a day of opening**.  **[1127](wp/1127-ladder-first-rung.md) closed 2026-08-22 — ✅, and it is the survey's speed side finally shipping something** (narrative in the record): the ladder rung, taken as *pricing* the first rung rather than choosing it, because choosing it was refuted before it was built — the collapse is still the better rung on **seven of the trigger case's nine** warm patterns, so its escalations are sporadic and generalising from one costs more on every pattern that did not need it.  A losing first rung does not cost *more* than a winning one, it costs **the whole budget** (`_collapse` takes the plan's largest `max_iter`), and the two do not overlap anywhere: 27-64 evaluations for a winner against ~400 for both losers.  So `first_rung_factor` (default **2.0**) bounds it at twice the dearest first rung the chain has already *converged*, and bounds nothing until one has: **trigger-series 1603 → 1331** evaluations (1315 → 1072 njev, 57.2-57.4 → 46.5-46.7 s), `cpd-series` identical to the evaluation, `direction="both"` on the small-cell pair 2839 → 2042 because its *backward* chain escalates.  **Answers bit-identical on both** — 0 of 1030 and 0 of 392 accepted values differ — because a bound only truncates a rung whose result is discarded and its replacement starts from the same warm state; two clauses keep it that way and **both were found by tests, not by the harness** (spending the bound must force the escalation, and `_prefer` must rank a truncated attempt below a completed one).  Two negatives outrank the ratio: the *attractive* second bound — "a warm refit costing more than the cold fit is not a warm refit", which needs no constant and reached **1.36×** — is **false**, since `_collapse` of a one-stage plan *is* that plan (cold 9 evaluations against warm 14 on the suite's own cheap plan, a **one-parameter toy a sharper adversary than either real dataset**), and no factor repairs it; and the WP's pre-registered clause 1 **fires** on the final rule (1331 against `refit="stages"`'s 1253), recorded as firing rather than reworded, with the judgement beside it that the clause was written for a rule that *chooses*.  Default flipped on the maintainer's call; `first_rung_factor=None` is the bit-identical way back.  The **per-reflection 19.4 % front stays unowned** (solver 11.8 %), and so now is the **`refit=` choice** — 1253 against 1331 says the mode outweighs the bound on the trigger case, 627 against 1041 says the reverse on the small-cell one, so it stays adaptive-or-nothing.
...
| [1127](wp/1127-ladder-first-rung.md) | The ladder's first rung: which one a warm pattern starts on | ✅ 2026-08-22 — bounded, not chosen; 1603 → 1331 evaluations with bit-identical answers | 1111, 1124, 1051 |
