...
**v1.2 — the GUI for a crystallographer** ([record](milestones/v1.2.md)); the maintainer's use notes triaged into 17 WPs (1201-1217) plus [1017](wp/1017-gui-manual-onboarding.md), the manual, last. Thirteen shipped — [1201](wp/1201-gui-house-style.md) (the token layer and nine registers), [1202](wp/1202-help-corpus.md) (`src/rietx/help.py`, the corpus), [1203](wp/1203-help-popover.md) (one popover mechanism), [1204](wp/1204-developer-mode-example-projects.md) and [1205](wp/1205-new-project-open-browse-defaults.md) (the empty state openable, and reaching any project), [1206](wp/1206-typed-cell-project.md) (**a project needs a lattice, not a structure**), [1207](wp/1207-pattern-only-project.md) (**a pattern is already a project**), [1208](wp/1208-plan-introduction.md) (**a plan is a ladder and it says what it will free**), [1209](wp/1209-peaks-table-numbers-flags.md) (**the peak table reads like a peak table**), [1210](wp/1210-peak-layer-identity.md) (**the peak layer stops borrowing**), [1211](wp/1211-candidate-overlay.md) (**a cell's claim, drawn over the pattern that has to answer it**), [1212](wp/1212-redraw-never-moves-axes.md) (**a redraw never moves the axes**) and [1213](wp/1213-hover-readout.md) (**the tooltip stops covering the data**) — and now [1214](wp/1214-model-vary-and-profile-save.md): **a refinement flag is set where the model is read**. Every value the Model panel shows carries the parameter table's own vary control beside it, riding the same `PATCH /api/params` as the value edits, one `set_vary` node per path; the phase's scale and its four sample-broadening terms got the controls they never had; and `POST /api/export/instrument_profile` is the export family's one member gated on the **model** rather than on a result, since a profile is what a calibration is frozen into. Three rules reach past the panel. **A held row's marks are one vocabulary and there are four of them** — `needs_held_cell` arrived after WP-1011's three and wore the mode-fixed mark in every project, because the glyph was a ternary whose last arm caught everything; an unknown reason still draws a mark, and a meta-test reads the fields `ParameterRow.refinable` actually tests so a fifth fails there first. **A field's parameter path is not always its model path prefixed** (`source.polarization` is `instrument.polarization` in θ, and was routing past `set_values`' bounds unnoticed). And **a width aimed at the value reaches the control beside it**, which drew an esd at zero width until the flag was excluded by name. What v1.2 does not cover yet: 47 authored tooltips remain elsewhere, per file, counted since 1203 (`rietx.help` has no arm for their vocabularies yet); the residual subplot's empty quarter, which 1212 measured and decided against collapsing; and the phase's corrections (extinction, preferred orientation, Stephens), which no form offers because offering one is offering to declare it. **Next: [1215](wp/1215-structure-table.md)** (Model — the structure table, which inherits an atom row of seven controls), then [1216](wp/1216-instrument-form.md), [1217](wp/1217-history-graph-compare.md) — each of which inherits its panel's tooltip count.
