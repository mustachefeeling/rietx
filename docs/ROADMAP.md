...
**v1.2 — the GUI for a crystallographer** ([record](milestones/v1.2.md)); the maintainer's use notes triaged into 17 WPs (1201-1217) plus [1017](wp/1017-gui-manual-onboarding.md), the manual, last. **The triage is done** — all seventeen shipped, one line each in the record — [1217](wp/1217-history-graph-compare.md) closing it: the history graph reads like a git graph and the compare table's numbers hold their columns. Three rules from it reach past the panel. **An edge holds a lane for its whole span**, which is a lane rule and not a drawing one: a tip-following pass frees a lane the moment its node is drawn, so a line still due to travel down it has nowhere to run and can only be a diagonal across the gap — reserved, a crossing is one row, and which end it crosses at follows from the reservation. **A `ch` column is one column only while every cell in it is one font at one size**, `ch` being the element's own zero; the family and the size go on the row. And **the formatter keeps the column's width**, falling back to exponential rather than writing anything longer, which is what lets three widths be stated once in TypeScript and handed to the CSS with no arithmetic to cross — WP-1216's rule where the numbers belong to code rather than to a stylesheet. What v1.2 does not cover: 47 authored tooltips elsewhere, per file, counted since 1203; the residual subplot's empty quarter, which 1212 measured and decided against collapsing; and the phase's corrections (extinction, preferred orientation, Stephens), which no form offers because offering one is offering to declare it. **Next: [1017](wp/1017-gui-manual-onboarding.md)** — the GUI manual, the in-app help anchors and the sync mechanism, the last of v1.2. Its screenshots inherit two widths: the model pane stacks below **1256 px**, and the history compare row fits a 559 px sidebar and side-scrolls a 380 px one.
