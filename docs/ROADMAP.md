...
**v1.2 — the GUI for a crystallographer** ([record](milestones/v1.2.md)): **all eighteen WPs are done** — the maintainer's use notes as 1201-1217, and [1017](wp/1017-gui-manual-onboarding.md), the manual, last. The milestone is complete and wants only its acceptance block written into the record. 1017's rules are about keeping prose true rather than about a panel, and three reach past the manual. **A vocabulary a document is *about* can be partitioned even when its prose cannot be checked**: the GUI's 77 routes and its nine panels are documented-or-excluded the way `tests/api_surface.py` partitions the call surface, and it tightens both ways — a chapter naming a route the server does not serve is WP-1037's bug pointed at the reader. **Where no python object knows the fact, the authorities swap**: nothing in the package knows the panels exist, so the tab strip is data in `gui/src/lib/tabs.ts`, vitest writes the corpus and pytest reads it, which is the fnmatch mechanism run backwards. And **a screenshot is generated or it is a lie with a timestamp** — `make_screenshots.py` drives the real server over a shipped example and is the one authority for how each was taken, so a moved control is a stale picture the build can name. Two smaller ones worth carrying: `*.png` in `.gitignore` has now swallowed a committed image family **three** times, so that is a test rather than a comment; and a count in prose rots exactly like a retuned threshold, which is how `using/quickstart.md` came to say three example projects over a listing of two. The GUI is no longer beta; its **routes** stay provisional by declaration, because the wire is the half that still moves. What v1.2 does not cover, unchanged: 47 authored tooltips elsewhere, the residual subplot's empty quarter (1212 measured it and decided against collapsing), and the phase's corrections, which no form offers because offering one is offering to declare it.
**Next: ship v1.2** — finish `milestones/v1.2.md` with its measured acceptance block and check README's claims. Then **v1.3 — agents and programs** (1301-1307, filed 2026-08-28), the agent surface refactored against two measured runs rather than intuition; it opens with the version bump. Free-standing peaks (1101-1103) stay at **v1.4**. `pyproject.version` is `1.2.0.dev0`.
