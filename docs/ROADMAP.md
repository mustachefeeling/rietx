...
**[1115](wp/1115-compiled-kernel-spike.md) shipped its tier 2026-08-22 — and it is the default install**: the gate read open on neither mechanism the WP was written around (dispatch is gone, the ragged axis costs 1.11×) but on **fusion and threading**, so `model/compiled.py` fuses the three kernels over `nogil` + a shared thread pool — trigger **17.6 → 8.9 s**, cpd-1a **4.2 → 2.2 s**, nac 0.54 → 0.40 s, Rwp equal to five decimals in all four.  The scatter is *bit-identical*; symmetric rows are too, in window, once each Ω spelling is transcribed faithfully (the 1120 difference is one association, and numba's `exp` matches numpy's bit for bit here); FCJ rows keep 1112's rounding bar.  **numba is a core dependency**, the user's call against the priced +157 MB and `numpy<2.6`: an extra cannot subtract a dependency, so the way out is a soft import plus `RIETX_COMPILED=0`, both reported by `capabilities().features`.  Cold JIT 0.60 s, 0.12 s cached, background-warmed from `compile_model`.  The cold target is still missed ~2×, and the front is now the 15 608 perturbed `phase_peaks` a Jacobian asks for — per-reflection scalars no plane kernel reaches, which is **[1121](wp/1121-per-reflection-cost.md)**, opened 2026-08-22 and the next thing to pick up.  It multiplies with the other half, now decided and in flight: **[1123](wp/1123-fast-tolerance-default.md)** flips 1113's priced tolerance schedule on — intermediate stages at ftol 1e-6 by default, 1.5–1.7× fewer whole-plan evaluations at ≤ 0.02 esd, `intermediate_ftol=None` the one switch back.  The stretch (< 1 s cold) has its one measured route owned too: **[1122](wp/1122-compiled-peaks-buffer.md)** — 1114's buffer design on 1115's substrate, the package's first *declared-tolerance* mode if it lands — gated on 1121's closing measurement.
