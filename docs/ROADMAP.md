...
**v1.3 — agents and programs** is in flight ([record](milestones/v1.3.md), 1301-1307 filed 2026-08-28): the agent surface refactored against two measured runs rather than intuition. [1301](wp/1301-hold-unsupported-phase.md) (2026-08-28) holds an unsupported phase's structural parameters for the stage rather than bounding them, its scale left free so it can still appear. [1302](wp/1302-error-is-documentation.md) (2026-08-29) makes the error the documentation — a wrong name answers with the right one across every schema, dataclass and the package itself — bounds a fit's diagnostics list, adds `progress=`, and gives a result a termination view answering "done or not, and why" in one call. [1303](wp/1303-retire-refine-json.md) (2026-08-29) deletes `rietx.agent`, so there is **one** integration surface and it is the python API: four traced rounds recorded zero calls of the JSON envelope from any agent with a choice, and it could not carry a series anyway (≈754 k tokens inline). `SCHEMA_VERSION` 0.11 → 0.12 is that ladder's first removal. [1304](wp/1304-protocol-as-skill.md) (2026-08-29) makes the protocol a **skill** in the open agentskills.io format — `docs/skill/rietx/`, a body read whole and nine references loaded on demand, in the wheel, on the site, and into another repository with `rietx skill --install`. The rule its own cap taught: **a lookup leaves the body, a rule and its decisive number stay**; written as tightly as it goes the material was 23 % over 32 kB, so §6's signal table, §4/§4b's evidence, §5 and the API index moved out while every rule kept the number that decides it. `SCHEMA_VERSION` 0.12 → 0.13 adds `capabilities().skill_path`, `docs/AGENT_PROTOCOL.md` is a pointer until v1.4, and `docs/releases/1.3.0.md` § Upgrading is the authority for both. **Next: [1305](wp/1305-series-deliverable.md)**, the series deliverable — which adds a row to a body with 375 bytes of headroom. Free-standing peaks (1101-1103) stay at **v1.4**.
