...
The format is an [open standard](https://agentskills.io/specification) rather
than one vendor's feature, so the same directory is read natively by Claude
Code, Codex, Cursor, Copilot, Gemini CLI, opencode, Goose, Cline, Amp, Kiro,
Devin, Zed, Junie, Qwen Code and OpenHands, among others. Nothing in it is
