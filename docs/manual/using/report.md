...
| `BackgroundEvidence.n_peaks` | explicit `HumpComponent` terms the fit declared | how much of the background's flexibility was localised peaks rather than a smooth curve. A projection of `RefinementResult.n_extra_components`, never a second count — including its null, which means nothing counted, as against 0, which means none was declared |
