...
| `BackgroundEvidence.n_peaks` | explicit `BackgroundPeak` terms the fit declared | how much of the background's flexibility was localised peaks rather than a smooth curve. A projection of `RefinementResult.n_background_peaks`, never a second count — including its null, which means nothing counted, as against 0, which means none was declared |
