...
This page exists because it is the name people guess.
...
a cost. {ref}`restraining-a-distance` is that reference, and it is a different
tool for a different job — a constraint you are sure of, a restraint you are
only mostly sure of.

The parameter *paths* both take, and the one grammatical trap in them, are in
[](model.md). The symmetry constraints you never declare — a cell edge
following another, a coordinate confined to its site-symmetry direction — are
created for you and read the same way, with `TieSpec.user` false.
