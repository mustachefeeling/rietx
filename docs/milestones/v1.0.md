...
`src/pxrdref/io/CLAUDE.md` said to: four formats, one `base.check_axis`, one
...
`src/pxrdref/io/CLAUDE.md` (loads under `io/`) and root kept the four
...
~190-line indexing dossier became `src/pxrdref/indexing/CLAUDE.md`,
...
`_editable_impl_pxrd_refine.pth` carrying the bare src path, not the
...
so a freshly opened `.pxrd` has no result and the plot draws the *raw* pattern —
...
made the `.pxt` pane a mode over the whole window because its content is
...
the `.pxt` document has **two** widths — its editable columns end at **546 px**
