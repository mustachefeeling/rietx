...
- [x] `anatase watch` (`watch.py` + `viz/live.py`): `LiveSession` writes `events.jsonl`, and rewrites `fit.html`/`status.json` per stage via an **atomic tmp-then-rename** so the poller never reads a torn file; the server is stdlib `http.server` with an auto-refresh page and a **console pane** tailing the event stream (no FastAPI, no websockets, no JS build).  `anatase` console-script entry point added
