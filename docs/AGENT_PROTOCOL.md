...
| `SIZE_UNUSUALLY_SMALL` | A phase's sample size broadening refined to an apparent crystallite below 5 nm (from `lor_size`, or `sqrt(gauss_size)`, via Scherrer at the pattern's wavelength). `where` names the term, `value` is the size in nm | **A number to check, not a failure** — the size twin of `STRAIN_UNUSUALLY_LARGE`, and also not a refusal. Real nanocrystals reach a few nm, so the fit may be right; but in our archive of 606 refinements the smallest well-determined crystallite is ≈ 33 nm and every refined size below ~2 nm is a divergence or placeholder signature. Confirm the broad lines are really size broadening and not strain, an unmodelled peak shape, an amorphous component or a second phase it is absorbing. If `BOUND_HIT` fired on the same path the width is held at the 2 nm floor and is **not** a measurement. A finite `Parameter.max` on the size term is your claim and silences both tiers |
