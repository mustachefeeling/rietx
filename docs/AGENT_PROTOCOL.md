...
| `CHI_X_AXIS_ASSUMED` | (warning — from the reader) Trust the 2θ scale of a `.chi` whose x-axis label named no axis this reader recognises; it was read as 2θ in degrees. A label that *is* recognisably q, d or a detector coordinate is **refused** rather than assumed, because converting it needs the wavelength it was integrated at and the file does not carry one — so this code means "unrecognisable", never "recognisably something else" |
...
| `RAS_ATTENUATOR_PRESENT` | (warning — from the reader) Compare intensities across the 2θ range this names. A Rigaku `.ras` carried an attenuator factor that is not 1, and it was **not** applied: no specification states whether the intensity column is already corrected for it, so applying it and not applying it are both defensible and guessing corrupts exactly the strong peaks Rietveld weights most. σ is affected too — √counts·attn is not √y. The message names the affected points; check the export against the instrument software before trusting their relative heights |
| `RAS_X_AXIS_ASSUMED` | (warning — from the reader) Trust the 2θ scale of a `.ras` whose `*MEAS_SCAN_AXIS_X` named no axis this reader recognises; the x column was read as 2θ in degrees. The `.chi` rule one format over: an axis that *is* recognisably something else — ω, χ, φ, θ, a height scan — is **refused**, because a rocking curve parses perfectly and refines to a confidently wrong cell. So this code means "unrecognisable", never "recognisably not 2θ" |
| `UXD_X_AXIS_ASSUMED` | (warning — from the reader) Trust the 2θ scale of a `.uxd` range whose `_DRIVE` names no axis this reader recognises; the stepped positions were read as 2θ in degrees. **The block marker is not evidence**: a rocking curve is stored under `_2THETACOUNTS` too, so the `_2THETA` prefix means "a position column is present", never "the positions are 2θ". A `_DRIVE` that *is* recognisably φ, χ, θ or a translation is refused. Most `.uxd` files in the wild are pole figures — 4 of the 5 real ones checked |
