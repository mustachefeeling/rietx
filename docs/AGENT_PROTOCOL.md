...
| `CAPILLARY_OFFSET_UNAVAILABLE` | (info) Read a cell from this fit as free of position systematics. The eq (4) offsets were held at zero for want of a goniometer radius, and an unmodelled offset moves every axis by the same fraction — so an axial ratio can agree to a few ppm while no axis does (measured: 143 ppm on both Cr2WO6 axes, c/a agreeing to 3.4 ppm) |
