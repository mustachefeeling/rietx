...
REFERENCES = HERE / "rietx" / "references"
#: The everyday index.  Every section renders here unless
#: :data:`TECHNIQUE_INDEX` routes it elsewhere.
TARGET = REFERENCES / "api.md"
#: Section title → the index file it renders into.  A technique most fits
#: never use pays for itself in every session's context if it sits in
#: ``api.md``, so it renders beside it under its own name (module docstring,
#: "`api.md` is the everyday index, and a technique gets one of its own").
#: This is authored, like ``SECTIONS`` itself; the file name is the whole of
#: the routing, because ``tests/test_skill.py`` reads the indexes as a glob.
TECHNIQUE_INDEX: dict[str, str] = {
    "An unknown magnetic structure": "api-magnetic.md",
}
...
SECTIONS: tuple[tuple[str, str, tuple[str, ...]], ...] = (
    ...
    (
        ...
        "`rx.solve_magnetic` is the same closed loop one subject over: on a "
        "converged **neutron** fit with unexplained intensity, it proposes a "
        "propagation vector, candidate models per k, one trial refinement "
        "per class the powder cannot separate, and a ranked list with a "
        "stated criterion — never Rwp (more amplitudes always fit better), "
        "so classes inside the tie width abstain together rather than name "
        "a winner. Refuses a non-neutron histogram by name; everything "
        "else, \"nothing here to solve\" included, is reported. "
        "Provisional: the criterion may still move.",
        ("rx.solve_magnetic",),
    ),
    (
        "Out",
...
#: The technique index's own opening.  ``{technique}`` is the section title
#: it was routed under, so the file says in its first line which sessions are
#: meant to load it.
TECHNIQUE_HEADER = """# The API index: {technique}
...
def indexes() -> tuple[str, ...]:
    """Every index file ``render`` can be asked for, ``api.md`` first."""
    out = ["api.md"]
    for title, _, _ in SECTIONS:
        name = TECHNIQUE_INDEX.get(title, "api.md")
        if name not in out:
            out.append(name)
    return tuple(out)


def render(index: str = "api.md") -> str:
    """The bytes of one generated index.

    ``index`` is a file name from :func:`indexes`.  A section renders into
    exactly one of them — :data:`TECHNIQUE_INDEX` says which — so the union
    over the indexes is ``SECTIONS`` exactly once, which is what
    ``tests/test_skill.py``'s coverage gate reads.
    """
    technique = next((t for t, n in TECHNIQUE_INDEX.items() if n == index), None)
    parts = [HEADER if technique is None
             else TECHNIQUE_HEADER.format(technique=technique)]
    for title, prose, names in SECTIONS:
        if TECHNIQUE_INDEX.get(title, "api.md") != index:
            continue
        parts += [f"## {title}", "", prose, ""]
