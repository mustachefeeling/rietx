...
const DIR = fileURLToPath(new URL(".", import.meta.url)), PORT = +(process.argv[2] ?? 8810);
...
http.createServer((q, r) => {
  let rel;   // a malformed %-escape throws, and a throw here would stop the server
  try { rel = decodeURIComponent(q.url.split("?")[0]).replace(/^\/$/, "/index.html"); } catch { r.writeHead(400); return r.end("bad path"); }
