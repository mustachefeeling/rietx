...
`covariance_estimates` ([`optimize/least_squares.py`](../../src/rietx/optimize/least_squares.py),
end of file) does:
...
1. **Reported physical esds are effectively RAW.** `stderr_physical`
   ([`params/vector.py`](../../src/rietx/params/vector.py)) rebuilds
...
2. **The high-correlation guard is dead.** `check_guards`
   ([`strategy/staged.py`](../../src/rietx/strategy/staged.py)) thresholds
...
([`optimize/least_squares.py`](../../src/rietx/optimize/least_squares.py),
