...
[`crystallography/lattice.py`](../../src/rietx/crystallography/lattice.py),
...
Cov is available in [`optimize/least_squares.py`](../../src/rietx/optimize/least_squares.py)),
...
[`schemas/results.py`](../../src/rietx/schemas/results.py) — one row per
...
- **2026-07-23** — **done.** Landed in five commits:
  ...
  2. `PhaseQuantity`/`QuantitativePhaseAnalysis` schema (re-exported from
     `rietx.schemas`) + optional `RefinementResult.qpa`.
