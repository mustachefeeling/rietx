...
- **`rietx compare` gets no new row, deliberately.** CLAUDE.md requires one
...
(`hasattr(rietx, "index")`), so the report panel's Apply button on that suggestion
...
- nothing else: **`features["indexing"]` needs no edit.** It is
  `hasattr(rietx, "index")`, a derived predicate, so it flips the moment
...
- [x] `index_pattern` + `IndexingResult` in `schemas/indexing.py`
      ...
      `Provenance`); `rietx/__init__.py` exports.
...
- **2026-07-30 — landed.** `indexing/consensus.py` (merge → Bravais → ambiguity →
  ...
  the `index_start`/`index_end` event kinds, `task="index"`, `rietx index`,
  ...
  **3. `high` confidence is currently unreachable on real lab data, by design, and
  ...
  - **`rietx compare` got no new row**, as the plan required, and the reasoning
