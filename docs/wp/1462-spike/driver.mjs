...
const cases = [["lab6", "ball"], ["nac", "ellipsoid"], ["fap", "ball"], ["nac", "ball"]];
...
for (const name of wanted) {
  ...
  for (const dpr of [2]) {
    ...
    for (const [s, mode] of cases) {
      ...
      await page.goto(`http://127.0.0.1:8823/index.html?s=${s}&mode=${mode}&rot=${process.env.ROT || 0.6}`);
      ...
      await page.locator("#wrap").screenshot({ path: join(here, "shots", `${name}_${s}_${mode}_dpr${dpr}.png`) });
      ...
      console.log(`${name.padEnd(8)} dpr${dpr} ${s.padEnd(4)} ${mode.padEnd(9)} atoms ${t.counts.atoms} halves ${t.counts.bondHalves}` +
