...
- `src/rietx/capabilities.py`: `capabilities()` → version, backends
...
- **`auto_background` and `diagnose` are unexported at the top level.**
  ...
  in `background.__init__.__all__` — but `rietx/__init__.py` never imports
  `background` at all. The GUI's "estimate background" button (and any
  scripting user) wants `rietx.auto_background`. Remember the invariant:
...
`title / description / modes / when_to_use`, exported as `rietx.PLAN_INFO`
...
`rietx/__init__.py` — the same top-level export gap this WP records for
...
- [x] `src/rietx/capabilities.py` + registry meta-test
...
- [x] Export `auto_background` and `diagnose` from `rietx.__init__`
...
- **2026-07-30** — **complete.** All four tasks plus one addition; 19 tests.
  ...
  *Done / decided:*
  ...
  - `features` are **derived predicates**, not literal booleans: schema-field
    ...
    and flips on its own when `rietx.index` lands, so nobody has to remember.
