...
- [ ] 1. `.claude/hooks/session_start.py` — the read-only scan, stdlib-only
...
- [ ] 2. `.claude/settings.json` — new checked-in file wiring
...
- [ ] 3. `tests/test_workflow_hooks.py` — one small file importing the scan
...
- [ ] 4. `.claude/commands/wp-start.md` — the session-start ritual: (a) act
...
- [ ] 5. Repair mode in `.claude/commands/wp-handover.md`: when invoked for
...
- [ ] 6. Protocol sync, minimal: ROADMAP § Session protocol step 1 gains
