...
- `src/rietx/io/readers.py`, `src/rietx/project.py`,
  `src/rietx/gui/imports.py`, `src/rietx/capabilities.py` — the seams.
...
- **2026-08-09 (session 4)** — **tasks 10-12 landed**, the three container
  ...
  **Two factorings, both at triggers a previous session named.** The axis policy
  ...
  `.xrdml` was the fourth consumer and `src/rietx/io/CLAUDE.md` said the fourth
  ...
  **Gotchas for the successor.**
  ...
  - **`src/rietx/io/CLAUDE.md`'s cap went 200 → 250** with three formats still
...
- **2026-08-09 (session 3)** — **tasks 8-9 landed**, `.ras` and `.uxd`, on
  ...
  **+9** (8 tests + one parametrized synthetic-fuzz row); and **+1 that is not a
  ...
  is parametrized over `SIZE_CAPS`, so adding `src/rietx/io/CLAUDE.md` to it
  ...
  **Structural change, decided with the user**: root CLAUDE.md was at exactly
  ...
  moved to **`src/rietx/io/CLAUDE.md`** (179 lines, capped at 200, loads under
