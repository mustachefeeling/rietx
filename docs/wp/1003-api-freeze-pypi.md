...
- **What that surface actually measures, built 2026-08-14: 1235 names, not
  the ~147 the WP predicted.** 127 classes, 902 fields, 151 methods, 27
  instance attributes, 25 functions, 2 constants, 1 module. Three derivation
  rules earn their keep and each was found by the guard *rejecting a correct
  page*, never by reading the code: declared-not-inherited membership (a bare
  `dir()` over the 47 exported classes is 1099 pydantic-machinery-dominated
  names); closure over reachable unexported types **including the return types
  of exported functions** (`Capabilities` is exported nowhere); and instance
  attributes read off the class source with `ast`, because a plain class
  assigns `self.history` in `__init__` where `dir()` on the class cannot see
  it. A freeze that enumerates by `dir()` alone would miss
  `Refinement.history`, `Project.doc` and `RefinementCancelled.node_id`. The
  bucket of not-yet-documented names is the generated
  `tests/api_surface_deferred.txt` — 1086 at the floor, regenerated by
  `python -m tests.api_surface --write-deferred`.
- **A fifth load-sensor row, measured 2026-08-14 and not fixed here.**
  `test_acceptance_indexing.py::test_a_short_clean_list_is_searched_ranked_and_reported_unscored`
  (fluorite) fails under `-n auto --dist loadgroup` on
  `assert all(res.search_complete[s] …)` and **passes serially in 207 s**.
  That is exactly the shape `tests/CLAUDE.md` already names four times — a
  completion assertion is a statement about machine load, not about the data,
  and belongs as "every system searched reports whether its domain was
  exhausted". WP-1067 added two test modules that session, and the v1.0
  record's own account of instance one is "adding one more test module was
  enough", so this WP is a plausible trigger and deliberately **not** the
  fixer: rewriting what an indexing acceptance row claims is a judgement about
  indexing, not about a manual. Fix the assertion, not the budget — the same
  file records that raising a budget bought 850 s and a 15:33 suite, and that
  narrowing what is searched is the lever.
- **The weekly `full` job has not completed since 2026-08-02, and a release
  cannot ship on a suite CI has not finished.** Found 2026-08-14 while looking
  for the count root CLAUDE.md § Numbers sends every session to fetch: the
  2026-08-09 run is `cancelled` at exactly 2h00m00s against
  `timeout-minutes: 120` (`.github/workflows/weekly.yml:40`), and the last
  success is 1h17m57s on 2026-08-02 — so the full suite grew past the ceiling
  in that week (the indexing acceptance and eval rows are the candidates) and
  **§ Numbers' designated source has been dead for twelve days.** It went
  unnoticed because a cancelled run is amber, not red. Raising the ceiling is
  the wrong first move: `tests/CLAUDE.md` already records that when a budget
