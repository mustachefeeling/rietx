...
- [x] 7. CLAUDE.md indexing block (~497-692) → ~25-line digest at root + new
  subsystem rulebook `src/rietx/indexing/CLAUDE.md`. The dossier's *rules*
  ...
  new indexing rule lands in `src/rietx/indexing/CLAUDE.md`; it earns a
...
- [x] 11. Pin the caps (LAST): `SIZE_CAPS` CLAUDE.md 720 → landed+~50
  ...
  `src/rietx/indexing/CLAUDE.md`. Rewrite the SIZE_CAPS comment block
...
- **2026-08-06 (close)** — all eleven tasks landed as eleven commits on
  ...
  *named skip* off darwin/arm64) and three new `SIZE_CAPS` parametrizations
  ...
  `src/rietx/indexing/CLAUDE.md` **212** (cap 250), gui 528 (cap 580),
