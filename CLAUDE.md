...
nothing until several have, never another fit's cost — the cold fit is the tempting
...
already being discarded and every accepted value stays bit-identical.  Set it
for **margin**: the gap between a winning rung and a losing one is ~6×, and a
factor calibrated tight enough to matter fired on a clean chain in CI only.
...
and no longer a beta feature; its **routes stay provisional by declaration**,
and they and the panel names are partitioned against those chapters by test.
...
- The **manual** (`docs/manual/`) is one `-W` Sphinx tree in two parts, each
  ...
  *built* HTML, and a diagram or a themed figure is checked by looking at it.
  ...
  saying so. Its **GUI chapters** (WP-1017) add a third guard, for a subject
  that is routes and panels rather than importable names: `test_gui_manual.py`
  **partitions** both vocabularies and tightens each way, so adding or renaming
  a GUI route or a tab fails until a chapter covers it, and naming one the
  server does not serve fails too. Their screenshots are generated the way the
  figures are, by `docs/manual/make_screenshots.py`.
