...
11. **Read the unmatched peaks.** `report.unmatched` with `kind="unmatched_obs"`
    ...
    mode only. Le Bail and Pawley extraction leaves no negative residual under an
    absent reflection, so there it fires on noise near a tick (61 % of
    reflections either way on a right cell and a doubled one).
