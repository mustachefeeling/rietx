...
| Bruker TOPAS `.inp` (project reader, `io/projects/topas.py`) | Read from the **file syntax only**, off a private archive of solved refinements — TOPAS itself is closed source and **no TOPAS code or macro library was consulted**. The archive is not redistributable; `tests/data/README.md` records its provenance and the facts it settled (block boundary, the four lattice-macro forms, the multi-`occ`/`adps`/`u11` incidences, the weight-percent oracle) | Everything the reader decides is this project's: the `_BLOCK` boundary, the one value grammar read once with its flag (`_read_tail`), report-or-refuse on a *stated but unreadable* cell key / `occ` / `beq` / ADP tensor, and the file-level `site`-count guard. The emission-profile macros (`CuKa5` and friends) are **not** expanded — only the anode is reported, and the wavelengths come from `rietx.schemas.instrument._RADIATIONS`. The two `grep -i topas` hits above are *physics* (Lobanov, the GSAS-II/TOPAS cylinder fit), reimplemented from papers; neither is this format. |
