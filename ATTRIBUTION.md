...
| Neutron TOF profile, incident spectrum and instrument-parameter readers (`model/profiles/tof.py`, `model/tof_spectrum.py`, `io/instrument_tof.py`) | Method references only; nothing ported | **Sources:** Von Dreele, Jorgensen & Windsor (1982), *J. Appl. Cryst.* 15, 581 (the back-to-back exponential ⊗ Gaussian profile and the incident-spectrum series); Ikeda & Carpenter (1985), *Nucl. Instrum. Methods A* 239, 536 (the moderator pulse, and why the full Ikeda-Carpenter shape is out of scope); Sabine (1988), *Acta Cryst.* A44, 368, and Sabine, Von Dreele & Jørgensen (1988), *Acta Cryst.* A44, 374 (TOF extinction); Larson & Von Dreele (2004), GSAS, LAUR 86-748, for the instrument-parameter file records and the coefficient laws; and the in-tree WP-1132 note (`docs/wp/1132-neutron-specimen-absorption.md`) for μ(λ) over Sears (1992) cross sections. **Process:** a literature-only specification was written 2026-09-22 by an agent given only those papers and this package's public API; the modules were implemented 2026-09-23 from that specification by an agent with no access to any reference implementation, to the specification writer's session, or to this package's previous TOF modules; those previous modules (written 2026-09-05 from the GSAS manual, whose author then read GSAS-II's `epsvoigt.for`, `hfunc.for`, `pypowder.for` and `GSASIIpwd.py` in the same session to compare the ΔT sign convention, and were read again 2026-09-06 to confirm the type-3 Lorentzian sign defect) were discarded in full and replaced by commit 5133a977. No GSAS or GSAS-II code, constant or digit was consulted for the rebuilt modules. The legacy LANSCE layout (`io/legacy/lansce_iparm.py`) is from our own instrument files, checked against GSAS-II v5.8.2 output as a black box, nothing read. The ITYP 1/2 fifth-pair exponent (P₁₀·exp(−P₁₁·T⁵), `model/tof_spectrum.py`), past the printed ladder of exponents (the manual's p. 128 with Von Dreele 1982 eqs. 4–5), was measured as 5.000000 (rms 2e-13) by conformance against GSAS-II 5.8.2's computed spectrum run as a black box on synthetic sets and the NPDF 7245 bank-2 block; no source was read. |
