...
it and listing the phases that do exist, so a typo is an error rather than a
column of blanks. And an agreement index gets **no `_esd` column at all**,
because it is a residual and has no esd in any series — where a column *is*
present and a cell is empty, that pattern did not estimate one, which is a
different fact. (Measured: WP-1310.)
