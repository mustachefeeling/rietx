...
| `.xy` / `.xye` / GSAS raw / pdCIF / FIT2D `.chi` / Rigaku `.ras` / Bruker `.uxd` readers, dispatched on content; a multi-scan file's scans are **selected** with `scan=`, never concatenated; a `.dif` peak list, a rocking curve and a binary vendor file are refused **by name**; instrument-profile files | ✅ |
