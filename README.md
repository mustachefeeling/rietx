...
`rietx` is an MIT-licensed Python package for Rietveld and Le Bail
...
at 1.23×, bit-identical; a JSON tool surface (`rietx.agent.refine_json`)
...
| matplotlib plots, **VLM montage**, plotly HTML viewer, `rietx watch` live view | ✅ |
...
```sh
rietx compare --open    # pick a standard, tick variants, read the Δχ² panel
```
