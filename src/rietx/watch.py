"""``rietx watch`` — live refinement viewer over stdlib http.server.

Serves a :class:`~rietx.viz.live.LiveSession` directory: the index page
...
and the viewer in another::
    ...
    rietx watch live/            # or: python -m rietx.watch live/
"""
...
<div id="bar">rietx watch — waiting for fit.html …</div>
...
def serve(directory: str | Path, *, port: int = 8899,
    ...
    print(f"rietx watch: serving {directory} at {url}  (Ctrl-C to stop)")
    if open_browser:
        import webbrowser
