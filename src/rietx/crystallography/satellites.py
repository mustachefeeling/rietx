    ...
    * both orders m = ±1 are enumerated and merged by orbit, which is what
    ...
    if len(q_rows) == 0:
        return ReflectionSet(
            ...
            operations=(sg.xyz if isinstance(sg, OperatorGroup) else None),
...
def merge_satellites(nuclear: ReflectionSet, sat: ReflectionSet) -> ReflectionSet:
    ...
    ``operations`` rides along with ``spacegroup``: a phase whose nuclear
    group has no Hermann-Mauguin symbol carries it as an explicit operation
    list, and the merged set needs it for the same reason the un-merged one
    does (a bracketed label with no list is not resolvable — see
    :func:`~rietx.crystallography.magnetic.scattering.merge_magnetic`, which
