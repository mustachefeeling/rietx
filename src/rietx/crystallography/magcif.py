...
- Gonzales-Platas, Katcho & Rodríguez-Carvajal (2021), *J. Appl. Cryst.* **54**,
  338 — the magCIF form as producers write it.
...
def propagation_vectors(text: str) -> tuple[tuple[str, str, str], ...]:
    ...
    Returned as three exact rational **strings** per vector — the spelling
    :attr:`rietx.schemas.structure.MagneticSymmetry.propagation_vector_parent`
    stores, and the spelling
    :attr:`~rietx.schemas.structure.Phase.propagation_vector` stores one field
    over — because k is exact and a float would put a tolerance where none
    belongs.  A component that is not a rational (``0.128``, the signature of
...
def read_magnetic_symmetry(block, text: str, path: str):
    ...
    names, the ``transform_BNS_Pp_abc`` string and the parent k carried as the
    metadata they are.
    ...
    return MagneticSymmetry(
        ...
        uni_number=uni,
        propagation_vector_parent=kvectors[0] if kvectors else None,
    )
...
def nuclear_operations(magnetic_symmetry) -> tuple[str, ...]:
    ...
    That is what makes the result usable as
    :attr:`~rietx.schemas.structure.Phase.symmetry_operations` directly rather
    than only as a hint: it is already closed under composition, already
    contains the identity, and — because it is read off the very strings the
    atoms' coordinates and moments are stated against — it is guaranteed to
    share their setting.  A symbol looked up in gemmi's table is not: gemmi's
    ...
    a right-looking name. See :func:`resolve_nuclear_symmetry`, which is what
    a caller should use to also get a label.
    """
...
def resolve_nuclear_symmetry(hm_symbol: str, magnetic_symmetry, path: str, *,
    ...
    """``(sg, label, symmetry_operations)`` for a magCIF's nuclear space group.

    ``hm_symbol`` — ``_parent_space_group.name_H-M_alt`` — names a symbol used
    only to give the phase a *label* and as a **cross-check**, never as the
    source of the operations: see :func:`nuclear_operations` for why a symbol
    lookup can silently disagree with the file's own setting.  ``sg`` is
    still gemmi's resolution of that symbol (the closest tabulated type),
    returned so a caller can use it for anything that only needs a type —
    ``structure_from_cif``'s cell-angle correction, for one.

    ``symmetry_operations`` is ``None`` where :func:`nuclear_operations`
    agrees with gemmi's tabulated operations for the symbol — bit-identical
    to every magCIF read before this function existed, since a phase with no
    explicit list resolves its operations from ``space_group`` exactly as it
    always did, and a symbol the tabulated set already reproduces earns
    nothing by restating the list. Only a **disagreement** carries the
    derived list explicitly (the one case a symbol lookup alone would get
    wrong), with the label bracketed —
    :func:`~rietx.crystallography.symmetry.unnamed_label`'s escape hatch,
    already defined by :class:`~rietx.schemas.structure.Phase` for a symbol
    that is only the closest type — and one ``CIF_MAGNETIC_NUCLEAR_SETTING``
    diagnostic appended to ``diagnostics`` when given, naming the file and the
    symbol so the substitution is reported rather than silently made — the
    same rule ``CIF_SPECIES_NORMALISED`` follows one column over.
    """
    import gemmi

    from ..schemas.structure import _op_key
    from .symmetry import get_spacegroup, unnamed_label
    ...
    derived = nuclear_operations(magnetic_symmetry)
    derived_keys = {_op_key(gemmi.Op(o)) for o in derived}
    tabulated_keys = {_op_key(op) for op in sg.operations()}
    if derived_keys == tabulated_keys:
        return sg, sg.xhm(), None
    label = unnamed_label(sg.xhm(), "unnamed in this cell")
    if diagnostics is not None:
        diagnostics.append(Diagnostic(
            level="info", code="CIF_MAGNETIC_NUCLEAR_SETTING",
            where=["phases.0.space_group"],
            message=(
                f"{path} names the parent space group {hm_symbol!r}, but "
                f"gemmi's tabulated operations for {sg.xhm()!r} do not "
                f"match this file's own magnetic operators with time "
                f"reversal dropped — the two settings' origins disagree "
                f"(as for Ima2/space group 46). The phase's symmetry is "
                f"built from the file's own operators; the symbol is "
                f"kept only as the closest type, bracketed in the label."),
            suggestion="this is a fact about the tabulated origin for "
                       "this Hermann-Mauguin symbol, not about the file: "
                       "nothing to change here"))
    return sg, label, derived
...
def write_magnetic_block(block, phase, *,
    ...
    * the ion and the Landé g under :data:`PRIVATE_TAGS`, which no magnetic
    ...
    if magnetic.propagation_vector_parent is not None:
        loop = block.init_loop("_parent_propagation_vector.", ["id", "kxkykz"])
        loop.add_row(["k1", "[" + " ".join(magnetic.propagation_vector_parent) + "]"])
