...
class Refinement:
    ...
    def parameters(self, *, mode: Mode | None = None) -> list[ParameterRow]:
        ...
        *ran* in, and before the first run that is the ``"rietveld"`` default —
        ...
        :class:`~anatase.project.Project`, from its document) has to be able to
    ...
    def suggest(self, data: PatternData, *, top_n: int = 5,
        ...
        *current state's*, seeds excluded.
        ...
        ``report`` takes a :class:`~anatase.report.FitReport` (or its
    ...
    def fit(self, data: PatternData, *, mode: Mode = "rietveld",
        ...
        :class:`~anatase.history.events.EventStream`.  See that module for the
        record format; ``anatase watch`` tails it live.

        ``cancel`` — an :class:`~anatase.optimize.cancel.CancelToken` another
    ...
    def report(self, *, plan: RefinementPlan | str | None = None, **kw):
        """The full :class:`~anatase.report.FitReport` for the last fit.
    ...
    def reflection_table(self) -> list["ReflectionRow"]:
        ...
        See :func:`anatase.io.exporters.reflection_table`.  In Le Bail/Pawley
    ...
    if record.out_of_range:
        out.append(Diagnostic(
            ...
            suggestion=("dilute the specimen, use a narrower capillary, or a "
                        "shorter wavelength — anatase.estimate_mu_r() shows "
...
def _roughness_regime_diagnostics(model: CompiledModel,
    ...
    see :class:`~anatase.schemas.instrument.RoughnessSuortti`); a test on ``b``
...
def estimate_mu_r(structure: Structure, instrument: Instrument) -> float | None:
    ...
    :mod:`anatase.crystallography.attenuation`) into a volume-fraction-weighted
    ...
    µR is not refinable, deliberately — see :mod:`anatase.model.absorption`.
    """
    geom = instrument.geometry
