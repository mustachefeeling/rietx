...
class Geometry(Base):
    ...
    Cryst.* A26, 682; see :mod:`anatase.model.absorption`), µR coming either
    ...
    * **sample transparency** — finite penetration moves the effective
    ...
    (:mod:`anatase.model.absorption`, *International Tables* Vol. C
    Table 6.3.3.1):
...
class BackgroundPSpline(Base):
    ...
    flag it (``anatase.background.diagnose``).
    """
...
class BackgroundFixedPlusChebyshev(Base):
    ...
    The fixed curve typically comes from :func:`anatase.background.estimate`
...
#: contamination check live in ``anatase.background.diagnostics``.
