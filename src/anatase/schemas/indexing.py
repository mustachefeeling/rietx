...
(:func:`anatase.crystallography.inv_d_squared`), which is what makes cell
...
#: (:func:`anatase.indexing.engines.search_line_order`).  No field changed and no
...
#: :mod:`anatase.indexing.peakfit`); recognising the alias is.
...
#: :func:`~anatase.indexing.pairs.pair_shift_sensitivity`, and using it put
...
#: by the first :data:`~anatase.indexing.engines.DEFAULT_SEARCH_LINES` lines and
...
#: constant read by :func:`anatase.indexing.consensus.confidence_for` rather than
...
#: :mod:`anatase.indexing.peakfit`).  ``excluded`` — the caller removed it.
...
#: :data:`~anatase.indexing.fom.MATCH_SIGMA` uses on positions, so one number
...
#: :data:`anatase.indexing.engines.DEFAULT_UNKNOWN_SHIFT_DEG`.
#:
...
class BravaisOpinion(Base):
    ...
    The serialisable face of :class:`anatase.indexing.reduce.BravaisScreen`, and
...
class ShiftScreen(Base):
    ...
    #: :mod:`anatase.indexing.pairs` recovered it from harmonic pairs with no
    #: reference at all; ``"unavailable"`` when neither was possible — see
    #: :func:`anatase.indexing.quality.assess_peak_list`.  Which of the first two
...
class IndexingResult(Base):
    """What :func:`anatase.index_pattern` returns — and what it *cannot* return.
...
class ExtinctionCandidate(Base):
    ...
    #: transcribed (see :func:`anatase.indexing.extinction.extinction_symbol`).
...
class ExtinctionScreen(Base):
    """What :func:`anatase.determine_extinction_symbol` returns.
