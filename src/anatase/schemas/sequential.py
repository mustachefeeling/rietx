...
``anatase.multi``).  What the user wants back is therefore not N unrelated
...
Following the history DAG's rule (see :mod:`anatase.schemas.history`), a
...
they are a few kB.  The full :class:`~anatase.schemas.results.RefinementResult`
of each pattern stays reachable in memory on the
:class:`~anatase.sequential.SequentialRefinement` that produced it.
"""
...
class SeriesResult(Base):
    ...
    def paths(self, *, varied_only: bool = False) -> list[str]:
        ...
        A :class:`~anatase.schemas.results.RefinementResult` records the
