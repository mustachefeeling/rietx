...
#: geometry — see :mod:`anatase.model.preferred_orientation`), and a value
...
class AnisoU(Base):
    ...
    one: :class:`~anatase.params.vector.ParameterTable` ties each component to
...
class PreferredOrientation(Base):
    ...
    transmission geometry) live in :mod:`anatase.model.preferred_orientation`.
...
class StephensStrain(Base):
    ...
    :mod:`anatase.crystallography.stephens`, which also derives the
