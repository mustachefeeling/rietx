...
A project is a **directory**, not an archive::
    ...
    my_sample.rex/
        ...
        live/               event streams for `anatase watch`
...
makes them trustworthy on re-open is :class:`~anatase.schemas.project.DataRef` —
...
class Project:
    ...
    def create(cls, path: str | Path, *, pattern: str | Path,
        ...
        ``reader_options`` are :data:`~anatase.io.readers.READER_OPTIONS` keys —
    ...
    def open(cls, path: str | Path, *, backend: str = "numpy",
        ...
        if major != PROJECT_FORMAT_VERSION.split(".")[0]:
            raise ValueError(
                ...
                f"another version of anatase (this one reads "
    ...
    def live_dir(self) -> Path:
        """Where an event stream belongs, so ``anatase watch`` can find it."""
