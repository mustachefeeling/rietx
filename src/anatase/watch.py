"""``anatase watch`` — live refinement viewer over stdlib http.server.

Serves a :class:`~anatase.viz.live.LiveSession` directory: the index page
...
and the viewer in another::
    ...
    anatase watch live/            # or: python -m anatase.watch live/
"""
...
_INDEX = """<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>anatase watch</title>
<style>
...
<div id="bar">anatase watch — waiting for fit.html …</div>
...
def serve(directory: str | Path, *, port: int = 8899,
    ...
    print(f"anatase watch: serving {directory} at {url}  (Ctrl-C to stop)")
    if open_browser:
        import webbrowser
...
if __name__ == "__main__":  # python -m anatase.watch <dir>
    main()
