"""The ``anatase`` command-line entry point.
...
``anatase index`` prints the **candidate list**, never one cell, and carries the
...
def main(argv: list[str] | None = None) -> int:
    ...
    if command == "html":
        if len(rest) != 2:
            print("usage: anatase html <result.json> <out.html>", file=sys.stderr)
            return 2
    ...
    print(f"anatase: unknown command {command!r} (try --help)", file=sys.stderr)
    return 2


def _index(argv: list[str]) -> int:
    """``anatase index`` — terminal-shaped because "what is this cell?" is a
