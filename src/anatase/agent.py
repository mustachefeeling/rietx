"""Single-call JSON surface for driving anatase from an agent (WP-0602).
...
- failure: ``{"ok": false, "error": {code, message, suggestion, details}}`` —
  the same grammar as :class:`~anatase.schemas.common.Diagnostic`, so an agent
...
:class:`~anatase.schemas.indexing.IndexingResult` is a different kind of answer,
...
class IndexRequest(Base):
    ...
    peaks: PeakList | None = Field(None, description=(
        "a fitted peak list (anatase.pick_peaks output, or "
