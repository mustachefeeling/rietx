...
having as validation (:mod:`anatase.indexing.workflow`, which owns it) and
...
:data:`~anatase.schemas.indexing.INDEX_REFUTING_CAVEATS` splits the rest in two:
...
class ConsensusOutcome:
    ...
    Separate from :class:`~anatase.schemas.indexing.IndexingResult` because this
...
def caveats_for(cand: CellCandidate, *, engines_run: Sequence[str],
    ...
    **``unmatched_observed`` is deliberately not read here, and it was measured
    ...
    (:func:`~anatase.indexing.fom.log_sum_scores`).
    """
