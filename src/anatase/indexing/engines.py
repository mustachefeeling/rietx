...
#: ``not_separable`` (:data:`~anatase.schemas.indexing.PEAK_REFUTED_SIGMA`) **both
...
#: :func:`~anatase.indexing.consensus.consensus` applies to the merged, ranked
#: answer, and the bound :func:`estimate_ceiling` prices validation by
#: (:func:`~anatase.indexing.consensus.checked_indices` never exceeds it).
#:
...
#: :func:`~anatase.indexing.consensus.grade`'s own floor, and since WP-1046 also
...
#: ``anatase.indexing.priors`` re-exports it, so it is still spelled
...
class SearchSpec:
    ...
    :func:`~anatase.indexing.quality.assess_peak_list` (Smith's envelope differs
    ...
    #: quantity :class:`~anatase.schemas.indexing.ShiftScreen` calls
    ...
    #: :class:`~anatase.schemas.indexing.IndexingResult` over whatever was
...
class EngineCandidate:
    ...
    Carries the :class:`~anatase.indexing.qspace.CandidateFit` rather than only
...
class Deadline(Budget):
    ...
    :class:`~anatase.optimize.cancel.CancelToken` does — which is the entire
    ...
    * :func:`~anatase.indexing.workflow.validate_by_lebail` re-raises
...
def provisional_payload(cands: Sequence[EngineCandidate]) -> list[dict]:
    ...
    snapshot in :func:`~anatase.indexing.workflow.index_pattern`, which uses
...
def effective_shift_allowance(spec: SearchSpec,
    ...
    :data:`~anatase.schemas.indexing.TRUSTED_SHIFT_SOURCES` names — against
...
def search_volume_ceiling(spec: SearchSpec, quality, system: str) -> float:
    ...
    :data:`~anatase.indexing.quality.VOLUME_ENVELOPE_SLACK`, because the
...
def match_window(peaks: PeakList, spec=None, quality=None) -> np.ndarray:
    ...
    :mod:`~anatase.indexing.consensus` and :mod:`anatase.viz.indexing` cannot
    derive it two ways.
    """
...
def rank_candidates(cands: Sequence[EngineCandidate], peaks: PeakList, *,
    ...
    **Whether two engines found a lattice outranks every figure of merit**
    ...
    :func:`~anatase.indexing.consensus.consensus` passes (WP-1046).  The
    ...
    they were never selected under (:func:`~anatase.indexing.fom.fom_panel`).
    """
...
def corroborated(cand: EngineCandidate) -> bool:
    ...
    :func:`~anatase.indexing.consensus.grade` floors a candidate below
...
def agreement(cand: EngineCandidate) -> int:
    ...
    ``found_by`` is empty until :func:`~anatase.indexing.consensus.consensus`
...
def estimate_ceiling(spec: SearchSpec | None = None, *,
    ...
    """What a run of :func:`~anatase.indexing.workflow.index_pattern` can cost,
    ...
    ``max_candidates`` — :func:`~anatase.indexing.consensus.checked_indices`
