...
:class:`anatase.schemas.indexing.PeakList` whose every line carries a *fitted*
...
peer of :func:`anatase.refine`.  It merges the engines' candidates as reduced
...
engines (:mod:`anatase.indexing.consensus`).  The rule the whole milestone is built
around is enforced by the return type rather than by documentation:
:class:`~anatase.schemas.indexing.IndexingResult` has no ``.cell``, and
