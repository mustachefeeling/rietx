...
time**.  So :class:`~anatase.schemas.indexing.ExtinctionCandidate` carries a
...
3. *Direct absence evidence refutes; the fit only ranks — but it must be read
   ...
   WP-1024's :func:`~anatase.indexing.workflow.absent_reflections` asks it
...
def line_index(two_theta: np.ndarray) -> tuple[np.ndarray, int]:
    ...
    tolerance is :data:`~anatase.indexing.fom.LINE_COINCIDENCE_RTOL`, nine orders
