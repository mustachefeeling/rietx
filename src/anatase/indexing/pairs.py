...
This module is what lets :func:`~anatase.indexing.quality.assess_peak_list` stop
...
:func:`~anatase.indexing.quality.template_collinearity`'s 0.96 arriving by a second
...
def shift_template(name: str, two_theta_deg: np.ndarray) -> np.ndarray:
    """One of :data:`~anatase.schemas.indexing.SHIFT_TEMPLATES`, at these angles.

    Same three curves :func:`~anatase.indexing.quality.shift_template_basis`
...
def pair_shift(two_theta_lo: np.ndarray, two_theta_hi: np.ndarray,
    ...
    :func:`~anatase.indexing.quality.fit_shift_model` fits and what
...
def pair_allowance(amplitude_deg: float, amplitude_esd_deg: float) -> float:
    ...
    **mean** — how well the amplitude is known — and never the pair-to-pair
    ...
    :data:`~anatase.schemas.indexing.SHIFT_ALLOWANCE_K_ESD` for the sweep.
