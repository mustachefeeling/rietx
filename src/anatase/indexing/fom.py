...
:class:`~anatase.schemas.indexing.FigureOfMerit` has a ``blind_spot`` field and it
...
#: :data:`~anatase.schemas.indexing.PEAK_MIN_USABLE_LINES` — an alias, not a
...
def predicted_lines(cell: tuple[float, ...], system: str, centring: str,
    ...
    **A line, not an orbit.**  ``generate_reflections`` returns one entry per
    ...
    (:func:`~anatase.indexing.qspace.centring_allows`), so no space-group
...
def log_sum_scores(panels: list[list[FigureOfMerit]], *,
    ...
    **This is not the ranking in use, and the measurement is why.**
    :func:`~anatase.indexing.engines.rank_candidates` still aggregates with
