...
:class:`~anatase.schemas.indexing.IndexingResult`, which has no ``.cell``, and the
...
def structure_from_candidate(candidate: CellCandidate, *,
    ...
    *Two: ``space_group`` defaults to the absence-free lattice group*, never to a
    ...
    :func:`anatase.indexing.extinction.determine_extinction_symbol`, which passes
...
def validate_by_lebail(candidate: CellCandidate, data: PatternData,
    ...
    :class:`~anatase.schemas.results.RefinementResult` the fit produced, as
    ...
    on** :class:`~anatase.schemas.indexing.LeBailValidation` for the reason
    ...
    **Le Bail rather than Pawley is a cost choice, not a discrimination one.**
    ...
    :class:`~anatase.model.forward.PawleyBlock` and its overlap restraints for
    ...
    **Except a cancellation, which is evidence about nothing** (WP-1037).
    ...
    :class:`~anatase.indexing.engines.Deadline`; either way the fit it stops is
...
def _stream_system_snapshot(progress, system: str, units: dict, names,
    ...
    (:func:`~anatase.schemas.indexing.candidate_evidence` — shared, never
...
def index_pattern(peaks: PeakList | None = None, *,
    ...
    Give it a :class:`~anatase.schemas.indexing.PeakList`, or a ``data`` +
    ...
    ``cancel`` is WP-1006's :class:`~anatase.optimize.cancel.CancelToken` and works
    ...
    slice.  It is enforced as a :class:`~anatase.indexing.engines.Deadline`,
    ...
    :class:`~anatase.schemas.indexing.IndexingResult` over what was reached.
