...
Refine one shared :class:`~anatase.schemas.structure.Structure` against several
patterns at once — different wavelengths, geometries or temperatures — each with
its own :class:`~anatase.schemas.instrument.Instrument`.  The histograms are
...
resolution.  See :mod:`anatase.params.multi` for the parameter-sharing map and
:func:`anatase.optimize.least_squares.run_multi_least_squares` for the stacked
solve.
...
class MultiHistogramRefinement:
    ...
    :class:`~anatase.params.multi.SharingMap`) overrides the default
