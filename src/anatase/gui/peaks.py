...
(:func:`~anatase.indexing.pick.peaks_of_group` — the same translation the
picker used, never a second reading) and their ghost marks
(:func:`~anatase.indexing.pick.flag_ghosts` with ``only=`` the new indices, so
...
provenance :func:`~anatase.indexing.pick._not_separable` needs, known only at
fit time).  The :class:`~anatase.indexing.peaks.Detection` itself is *not*
...
def set_flags(doc: PeakDoc, index: int, *, use_for_indexing: bool | None = None,
    ...
    :data:`~anatase.schemas.indexing.PEAK_UNUSABLE_FLAGS` from this peak: that
...
class PeakEditor:
    ...
    def refit(self, doc: PeakDoc, group: int, *,
        ...
        (:func:`~anatase.indexing.peakfit.fit_group`: shoulder pruning and
    ...
    def curves(self, doc: PeakDoc) -> list[dict]:
        ...
        (:func:`~anatase.indexing.peakfit.group_profile`), so drawing costs no
