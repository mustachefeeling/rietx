...
(:mod:`anatase.sequential`), not one joint residual, and this module is where
...
:class:`~anatase.schemas.common.Diagnostic`, which carries ``where`` and no
...
class SeriesMember:
    ...
    #: (:data:`~anatase.io.readers.READER_OPTIONS`), because a member of a series
...
def check_settings(carry: Any, refit: Any, direction: Any) -> None:
    ...
    ``REFIT_MODES``/``DIRECTIONS`` are :mod:`anatase.sequential`'s own tuples, so
...
def result_payload(series: SeriesResult, backward: SeriesResult | None, *,
    ...
    full :class:`~anatase.schemas.results.RefinementResult` — a
