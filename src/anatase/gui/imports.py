...
class UploadRefused(ValueError):
    ...
    Same grammar as :class:`~anatase.gui.session.GuiError`, which is what wraps
...
def preview_pattern(upload: Upload, *, reader_options: dict[str, Any] | None = None,
    ...
    ``reader_options`` are :data:`~anatase.io.readers.READER_OPTIONS` keys, and
    ...
    return {
        **upload.as_dict(),
        ...
        # one; the server's working directory is where ``anatase gui`` was
...
def preview_instrument(upload: Upload) -> dict:
    ...
    try:
        ...
        raise UploadRefused(
            f"{upload.filename} is not a anatase instrument profile: "
            f"{type(exc).__name__}: {scrub(str(exc), upload)}") from None
...
def unknown_species(structure) -> list[dict]:
    ...
    A :class:`~anatase.schemas.structure.Structure` validates fine with a species
