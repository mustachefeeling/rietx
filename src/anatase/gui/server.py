"""``anatase gui`` — the localhost HTTP surface over a :class:`GuiSession`.

Stdlib ``http.server``, as ``anatase watch`` and ``anatase compare`` already are.
...
:class:`~anatase.gui.session.GuiSession` method, serialise — so replacing it
...
def _upload_options(query: dict) -> dict:
    ...
    :data:`~anatase.io.readers.READER_OPTIONS` rather than a literal list — so a
            ...
            if not self._origin_ok():
                self._json({"error": {
                    ...
                    "message": f"{self.headers.get('Host')!r} is not a loopback "
                               "host; anatase gui serves 127.0.0.1 only",
                    "where": []}}, 403)
                return
...
def serve(session: GuiSession, *, port: int = DEFAULT_PORT,
    ...
    if machine:
        ...
        print(f"anatase gui — {url}", flush=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="anatase gui",
        description="Serve the anatase GUI for a project on localhost.")
    ...
    parser.add_argument("--backend", default="numpy",
                        help="Jacobian backend (see anatase.capabilities())")
...
<html><head><meta charset="utf-8"><title>anatase gui</title>
<style>
...
<h1>anatase gui — server running, frontend not built</h1>
<p class="sub">The Svelte app is WP-1010. The HTTP API below is live now, and
every route is a method on <code>anatase.gui.GuiSession</code>.</p>
<ul>
