"""``anatase compare`` — a browser UI for comparing refinement settings.

    anatase compare              # → http://127.0.0.1:8730
    anatase compare --open --port 9000 --data /path/to/tests/data
...
and the page draws three linked panels — see :mod:`anatase.viz.compare` for
what each one answers and why the third one is the load-bearing view.

Same architecture as ``anatase watch``: stdlib ``http.server``, plain fetch
...
def _handler(state: _State):
    ...
    class Handler(http.server.BaseHTTPRequestHandler):
        server_version = "anatase-compare"
...
<html><head><meta charset="utf-8"><title>anatase — compare settings</title>
...
<div id="side">
  <h1>anatase · compare settings</h1>
