...
def reflection_table(model: CompiledModel, values: dict[str, float],
    ...
    parameter dict (see :meth:`anatase.Refinement.reflection_table`, which wires
...
def write_refinement_cif(result: RefinementResult, structure: Structure,
    ...
    pdCIF tags :func:`anatase.read_pdcif` reads and the structure block the tags
    :func:`anatase.Structure.from_cif` reads, so the file round-trips through the
