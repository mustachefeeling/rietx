...
state.  **Not** one joint residual — that is :mod:`anatase.multi`, which stacks
...
produced each point.  :class:`~anatase.schemas.sequential.SeriesResult` is that
...
:data:`~anatase.history.events.EVENT_SCHEMA_VERSION` does not move — the rule is
...
class _SeriesStream(EventStream):
    ...
    :func:`~anatase.history.events.as_event_stream`, which passes an
...
class SequentialRefinement:
    ...
    *initial* models on every pattern.
    ...
    :class:`~anatase.schemas.results.RefinementResult` objects (with curves,
    for plotting) and :attr:`trees_` the per-pattern histories; the returned
    :class:`~anatase.schemas.sequential.SeriesResult` carries the summaries and
