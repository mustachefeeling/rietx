...
:class:`~anatase.schemas.plan.StageSpec` and executes nothing, so applying a
...
* ``increase_background_flexibility`` / ``decrease_background_flexibility`` —
  ...
  *evidence*, not the kind: a more flexible background lowers Rwp while biasing
  ...
  :func:`~anatase.optimize.statistics.background_absorption` — reached a caller
  ...
  emitter (:func:`~anatase.report.layer2.background_actions`); before that they
...
class Recipe:
    """How one :data:`~anatase.report.schemas.ActionKind` is carried out.
