...
what the gated linear misfit attribution of :mod:`anatase.report.layer1`
...
def lebail_gap(model, values: dict[str, float], *, rwp_rietveld: float,
    ...
    :class:`~anatase.report.schemas.LeBailGap` docstring.
...
def background_clause(bg: BackgroundEvidence) -> str | None:
    ...
    section publishes: :class:`~anatase.report.schemas.BackgroundEvidence` is
...
def build_layer0(result: RefinementResult, *, top_n: int = 15,
    ...
    """Layer 0 only.  :func:`anatase.build_report` adds Layers 1-2 on top."""
