...
#: :data:`~anatase.strategy.staged.BACKGROUND_ABSORPTION_GUARD` itself rather
...
class BackgroundEvidence(Base):
    ...
    **Too flexible** — the background imitates the peaks, biasing ADPs up and
    ...
    (:class:`~anatase.schemas.results.Identifiability`).  Pairwise ρ cannot see
...
class ExchangeFinding(Base):
    ...
    :class:`~anatase.schemas.results.ExchangeRow`) plus the report's half of
    ...
    zero — :data:`~anatase.optimize.identifiability.NULL_IDENTITY`), and
    ...
    reading share (:func:`~anatase.report.identifiability.is_exchangeable`):
...
class IdentifiabilityEvidence(Base):
    ...
    (:class:`~anatase.schemas.results.Identifiability`); ``None`` here means
...
class LeBailGap(Base):
    ...
    **Convention.**  ``rwp_lebail`` is measured by running
    :meth:`~anatase.model.forward.CompiledModel.lebail_update` for
...
class TextureAnalysis(Base):
    ...
    :mod:`anatase.model.preferred_orientation`).  ``r2`` is the fraction of
...
class SuggestedAction(Base):
    ...
    per action**: :func:`~anatase.report.build_report` computes
