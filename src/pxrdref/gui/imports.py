...
what went wrong rather than a half-built ``.pxrd`` to clean up.  This is the same
...
class UploadRefused(ValueError):
    ...
    Same grammar as :class:`~pxrdref.gui.session.GuiError`, which is what wraps
...
def preview_pattern(upload: Upload, *, reader_options: dict[str, Any] | None = None,
    ...
    return {
        **upload.as_dict(),
        ...
        # one; the server's working directory is where ``pxrdref gui`` was
...
def preview_instrument(upload: Upload) -> dict:
    ...
    try:
        ...
        raise UploadRefused(
            f"{upload.filename} is not a pxrdref instrument profile: "
            f"{type(exc).__name__}: {scrub(str(exc), upload)}") from None
...
def unknown_species(structure) -> list[dict]:
    ...
    A :class:`~pxrdref.schemas.structure.Structure` validates fine with a species
