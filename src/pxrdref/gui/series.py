...
(:mod:`pxrdref.sequential`), not one joint residual, and this module is where
...
:class:`~pxrdref.schemas.common.Diagnostic`, which carries ``where`` and no
...
def check_settings(carry: Any, refit: Any, direction: Any) -> None:
    ...
    ``REFIT_MODES``/``DIRECTIONS`` are :mod:`pxrdref.sequential`'s own tuples, so
...
def result_payload(series: SeriesResult, backward: SeriesResult | None, *,
    ...
    full :class:`~pxrdref.schemas.results.RefinementResult` — a
