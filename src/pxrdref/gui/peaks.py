...
(WP-1005's ``.pxrd/`` container, ``peaks.json``), keyed by ``data_fingerprint``
...
(:func:`~pxrdref.indexing.pick.peaks_of_group` — the same translation the
picker used, never a second reading) and their ghost marks
(:func:`~pxrdref.indexing.pick.flag_ghosts` with ``only=`` the new indices, so
...
provenance :func:`~pxrdref.indexing.pick._not_separable` needs, known only at
fit time).  The :class:`~pxrdref.indexing.peaks.Detection` itself is *not*
...
def set_flags(doc: PeakDoc, index: int, *, use_for_indexing: bool | None = None,
    ...
    :data:`~pxrdref.schemas.indexing.PEAK_UNUSABLE_FLAGS` from this peak: that
...
class PeakEditor:
    ...
    def refit(self, doc: PeakDoc, group: int, *,
        ...
        (:func:`~pxrdref.indexing.peakfit.fit_group`: shoulder pruning and
    ...
    def curves(self, doc: PeakDoc) -> list[dict]:
        ...
        (:func:`~pxrdref.indexing.peakfit.group_profile`), so drawing costs no
